...
- `codeforge web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `codeforge web` view. **Do not** add for terminal, TUI, or general codeforge issues.
...
**Only** add if the issue mentions "zen" or "codeforge zen" or "codeforge black".

If the issue doesn't have "zen" or "codeforge black" in it then don't add zen label

#### core

Use for core server issues in `packages/codeforge/`, excluding `packages/codeforge/src/cli/cmd/tui/`.
...
TUI (`packages/codeforge/src/cli/cmd/tui/...`):
...
Core (`packages/codeforge/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for codeforge server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
