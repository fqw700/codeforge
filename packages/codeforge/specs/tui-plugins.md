...
- Author package entrypoint is `@codeforge-ai/plugin/tui`.
...
```json
{
  "$schema": "https://YOUR_DOMAIN.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/codeforge-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@codeforge-ai/plugin/tui`.
- `@codeforge-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@codeforge-ai/plugin/tui"
...
- The value is a semver range checked against the running CodeForge version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.codeforge` is absent, no check is performed (backward compatible).
...
- `codeforge plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `codeforge plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.codeforge` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.codeforge`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.codeforge`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.codeforge` dir or `OPENCODE_CONFIG_DIR`, CodeForge installs `@codeforge-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@codeforge-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.codeforge/themes` area near the plugin config source.
...
- Local smoke plugin: `.codeforge/plugins/tui-smoke.tsx`
- Local smoke config: `.codeforge/tui.json`
- Local smoke theme: `.codeforge/plugins/smoke-theme.json`
