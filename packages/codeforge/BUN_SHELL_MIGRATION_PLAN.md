...
Replace runtime Bun shell template-tag usage in `packages/codeforge/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/codeforge/src` is removed except:
