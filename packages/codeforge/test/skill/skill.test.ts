...
test("discovers skills from .codeforge/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".codeforge", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".codeforge", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".codeforge", "skill", "dir-skill")
...
test("discovers multiple skills from .codeforge/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".codeforge", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".codeforge", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".codeforge", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const codeforgeSkillDir = path.join(dir, ".codeforge", "skill", "agent-skill")
      const codeforgeSkillsDir = path.join(dir, ".codeforge", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(codeforgeSkillDir, "SKILL.md"),
        `---
name: codeforge-skill
description: A skill in the .codeforge/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(codeforgeSkillsDir, "SKILL.md"),
        `---
name: codeforge-skill
description: A skill in the .codeforge/skills directory.
---
