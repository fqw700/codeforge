...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const codeforgeModel = {
      ...openaiModel,
      providerID: "codeforge",
      api: {
        id: "codeforge-test",
        url: "https://api.codeforge.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, codeforgeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.codeforge?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.codeforge?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          codeforge: { itemId: "msg_codeforge" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              codeforge: { itemId: "msg_codeforge_part" },
    ...
    expect(result[0].providerOptions?.codeforge?.itemId).toBe("msg_codeforge")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.codeforge?.itemId).toBe("msg_codeforge_part")
