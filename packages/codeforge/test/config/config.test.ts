...
async function writeManagedSettings(settings: object, filename = "codeforge.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "codeforge.json") {
...
test("ignores legacy tui keys in codeforge config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://YOUR_DOMAIN.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "codeforge.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { codeforge: { options: { apiKey: "{env:OPENCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["codeforge"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "codeforge.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .codeforge directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codeforgeDir = path.join(dir, ".codeforge")
      await fs.mkdir(codeforgeDir, { recursive: true })
      const agentDir = path.join(codeforgeDir, "agent")
...
test("loads agents from .codeforge/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codeforgeDir = path.join(dir, ".codeforge")
      await fs.mkdir(codeforgeDir, { recursive: true })

      const agentsDir = path.join(codeforgeDir, "agents")
...
test("loads commands from .codeforge/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codeforgeDir = path.join(dir, ".codeforge")
      await fs.mkdir(codeforgeDir, { recursive: true })

      const commandDir = path.join(codeforgeDir, "command")
...
test("loads commands from .codeforge/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codeforgeDir = path.join(dir, ".codeforge")
      await fs.mkdir(codeforgeDir, { recursive: true })

      const commandsDir = path.join(codeforgeDir, "commands")
...
test("installs dependencies in writable OPENCODE_CONFIG_DIR", async () => {
  ...
  const run = spyOn(BunProc, "run").mockImplementation(async (_cmd, opts) => {
    const mod = path.join(opts?.cwd ?? "", "node_modules", "@codeforge-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@codeforge-ai/plugin", version: "1.0.0" }),
    )
    return {
      code: 0,
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "codeforge.json"),
        JSON.stringify({ $schema: "https://YOUR_DOMAIN.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .codeforge config
      const projectDir = path.join(dir, "project")
      const codeforgeDir = path.join(projectDir, ".codeforge")
      await fs.mkdir(codeforgeDir, { recursive: true })
      ...
      // Local .codeforge config with different plugins
      await Filesystem.write(
        path.join(codeforgeDir, "codeforge.json"),
        JSON.stringify({
          $schema: "https://YOUR_DOMAIN.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .codeforge config with some overlapping plugins
      await Filesystem.write(
        path.join(codeforgeDir, "codeforge.json"),
        JSON.stringify({
          $schema: "https://YOUR_DOMAIN.ai/config.json",
...
test("local .codeforge config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .codeforge directory config enables it
      const codeforgeDir = path.join(dir, ".codeforge")
      await fs.mkdir(codeforgeDir, { recursive: true })
      await Filesystem.write(
        path.join(codeforgeDir, "codeforge.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/codeforge")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/codeforge")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "codeforge.json")
    expect(await Config.resolvePluginSpec("oh-my-codeforge@2.4.3", file)).toBe("oh-my-codeforge@2.4.3")
...
describe("deduplicatePlugins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-codeforge@2.4.3", "file:///project/.codeforge/plugin/oh-my-codeforge.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.codeforge/plugin/demo.ts", "file:///project/.codeforge/plugin/demo.ts"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result).toEqual(["file:///project/.codeforge/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const codeforgeDir = path.join(projectDir, ".codeforge")
        const pluginDir = path.join(codeforgeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .codeforge/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .codeforge directory with a command
          const codeforgeDir = path.join(dir, ".codeforge", "command")
          await fs.mkdir(codeforgeDir, { recursive: true })
          await Filesystem.write(path.join(codeforgeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .codeforge should NOT be in directories list
