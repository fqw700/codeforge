...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  const dirpath = sanitizePath(path.join(os.tmpdir(), "codeforge-test-" + Math.random().toString(36).slice(2)))
  await fs.mkdir(dirpath, { recursive: true })
  if (options?.git) {
    await $`git init`.cwd(dirpath).quiet()
    await $`git config core.fsmonitor false`.cwd(dirpath).quiet()
    await $`git config user.email "test@codeforge.test"`.cwd(dirpath).quiet()
    await $`git config user.name "Test"`.cwd(dirpath).quiet()
    await $`git commit --allow-empty -m "root commit ${dirpath}"`.cwd(dirpath).quiet()
  }
  if (options?.config) {
    await Bun.write(
      path.join(dirpath, "codeforge.json"),
      JSON.stringify({
...
export function tmpdirScoped(options?: { git?: boolean; config?: Partial<Config.Info> }) {
  return Effect.gen(function* () {
    ...
    const dir = yield* fs.makeTempDirectoryScoped({ prefix: "codeforge-test-" })
    ...
    if (options?.git) {
      ...
      yield* git("config", "user.email", "test@codeforge.test")
    ...
    if (options?.config) {
      yield* fs.writeFileString(
        ...
        JSON.stringify({ $schema: "https://YOUR_DOMAIN.ai/config.json", ...options.config }),
      )
    }
