...
describe("plugin.install.task", () => {
  test("writes both server and tui config entries", async () => {
    ...
    const server = await read(path.join(tmp.path, ".codeforge", "codeforge.jsonc"))
    const tui = await read(path.join(tmp.path, ".codeforge", "tui.jsonc"))
  ...
  test("does not change configured package version without force", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".codeforge", "codeforge.json")
  ...
  test("writes to global scope when global flag is set", async () => {
    ...
    expect(await Filesystem.exists(path.join(global, "codeforge.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".codeforge", "codeforge.jsonc"))).toBe(false)
  })
  ...
  test("writes local scope under directory when vcs is not git", async () => {
    ...
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".codeforge", "codeforge.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(worktree, ".codeforge", "codeforge.jsonc"))).toBe(false)
  })
  ...
  test("writes tui local scope under directory when worktree is root slash", async () => {
    ...
    const ok = await run(ctxRoot(directory))
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".codeforge", "tui.jsonc"))).toBe(true)
  })
  ...
  test("writes only tui config for tui-only plugins", async () => {
    ...
    const ok = await run(ctx(tmp.path))
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".codeforge", "tui.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".codeforge", "codeforge.jsonc"))).toBe(false)
  })
  ...
  test("force replaces version in both server and tui configs", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server", "tui"])
    const server = path.join(tmp.path, ".codeforge", "codeforge.json")
    const tui = path.join(tmp.path, ".codeforge", "tui.json")
  ...
  test("returns false and keeps config unchanged for invalid JSONC", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".codeforge", "codeforge.jsonc")
    await fs.mkdir(path.dirname(cfg), { recursive: true })
  ...
  test("returns false when manifest declares no supported targets", async () => {
    ...
    expect(ok).toBe(false)
    expect(await Filesystem.exists(path.join(tmp.path, ".codeforge", "codeforge.jsonc"))).toBe(false)
    expect(await Filesystem.exists(path.join(tmp.path, ".codeforge", "tui.jsonc"))).toBe(false)
  })
