...
export const DEPRECATED_PLUGIN_PACKAGES = ["codeforge-openai-codex-auth", "codeforge-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, codeforgeVersion: string) {
  if (!semver.valid(codeforgeVersion) || semver.major(codeforgeVersion) === 0) return
  const pkg = await readPluginPackage(target).catch(() => undefined)
  if (!pkg) return
  const engines = pkg.json.engines
  if (!isRecord(engines)) return
  const range = engines.codeforge
  if (typeof range !== "string") return
  if (!semver.satisfies(codeforgeVersion, range)) {
    throw new Error(`Plugin requires codeforge ${range} but running ${codeforgeVersion}`)
  }
}
