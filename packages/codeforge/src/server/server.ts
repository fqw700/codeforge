  ...
  export const ControlPlaneRoutes = (opts?: { cors?: string[] }): Hono => {
    ...
    return app
      ...
      .use((c, next) => {
        ...
        const username = Flag.OPENCODE_SERVER_USERNAME ?? "codeforge"
      ...
      .use(
        cors({
          ...
          origin(input) {
            ...
            // *.codeforge.ai (https only, adjust if needed)
            if (/^https:\/\/([a-z0-9-]+\.)*codeforge\.ai$/.test(input)) {
              return input
            }
