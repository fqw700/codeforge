...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@codeforge-ai/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `codeforge auth login` in the terminal",
        name: "Login with codeforge",
        id: "codeforge-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // CodeForge stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const codeforgeProvider = providers.find((p) => p.id === "codeforge")
    if (codeforgeProvider) {
      if (codeforgeProvider.models["big-pickle"]) {
        return { providerID: ProviderID.codeforge, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(codeforgeProvider.models))
      if (best) {
        return {
