...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for codeforge.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal codeforge sessions
...
codeforge acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 codeforge acp
```
...
- **Terminal Integration**: Full terminal support via codeforge's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | codeforge acp
```
...
### Mapping to CodeForge

ACP sessions map cleanly to codeforge's internal session model:
