...
export namespace Project {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@codeforge/Project") {}
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
        return yield* fs.readFileString(pathSvc.join(dir, "codeforge")).pipe(
          Effect.map((x) => x.trim()),
      ...
      const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
        ...
        const data: DiscoveryResult = yield* Effect.gen(function* () {
          ...
          if (!id) {
            ...
            if (id) {
              yield* fs.writeFileString(pathSvc.join(worktree, ".git", "codeforge"), id).pipe(Effect.ignore)
            }
          }
