...
import codeforge from "./theme/codeforge.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@codeforge-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "codeforge")
        draft.active = typeof active === "string" ? active : "codeforge"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.codeforge, store.mode)
    })
