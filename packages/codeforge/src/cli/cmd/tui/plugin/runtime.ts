...
function createThemeInstaller(meta: TuiConfig.PluginMeta, root: string, spec: string): TuiTheme["install"] {
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".codeforge"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".codeforge", "themes")
...
function defaultPluginMeta(state: RuntimeState): TuiConfig.PluginMeta {
  return {
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".codeforge", "tui.json"),
  }
}
