...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall codeforge and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall CodeForge")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g codeforge-ai",
      pnpm: "pnpm uninstall -g codeforge-ai",
      bun: "bun remove -g codeforge-ai",
      yarn: "yarn global remove codeforge-ai",
      brew: "brew uninstall codeforge",
      choco: "choco uninstall codeforge",
      scoop: "scoop uninstall codeforge",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "codeforge", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using CodeForge!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# codeforge") || content.includes(".codeforge/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".codeforge/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".codeforge/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".codeforge"))
    ) {
      continue
    }
