...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run codeforge",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for codeforge session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found codeforge session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["codeforge", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting codeforge...")
        UI.println()

        const codeforgeArgs = sessionId ? ["-s", sessionId] : []
        const codeforgeProcess = Process.spawn(["codeforge", ...codeforgeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await codeforgeProcess.exited
        if (code !== 0) throw new Error(`codeforge exited with code ${code}`)
      },
    })
  },
})
