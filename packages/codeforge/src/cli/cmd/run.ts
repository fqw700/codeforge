...
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@codeforge-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run codeforge with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running codeforge server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
  ...
  handler: async (args) => {
    ...
    if (args.attach) {
      const headers = (() => {
        ...
        const username = process.env.OPENCODE_SERVER_USERNAME ?? "codeforge"
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://codeforge.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
