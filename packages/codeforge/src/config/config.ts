...
import { NamedError } from "@codeforge-ai/util/error"
...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/codeforge"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "codeforge")
      default:
        return "/etc/codeforge"
    }
  }
  ...
  export async function needsInstall(dir: string) {
    ...
    const mod = path.join(dir, "node_modules", "@codeforge-ai", "plugin")
    ...
    const depVersion = dependencies["@codeforge-ai/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      if (!online()) return false
      const stale = await PackageRegistry.isOutdated("@codeforge-ai/plugin", depVersion, dir)
      if (!stale) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: "@codeforge-ai/plugin",
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.codeforge/command/", "/.codeforge/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.codeforge/agent/", "/.codeforge/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
   ...
   * 2. Local codeforge.json
   * 3. Global plugin/ directory
   * 4. Global codeforge.json
   *
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: codeforge.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for codeforge serve and web commands"),
      command: z
        .record(z.string(), Command)
        .optional()
        .describe("Command configuration, see https://YOUR_DOMAIN.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://YOUR_DOMAIN.ai/docs/agents"),
      provider: z
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@codeforge/Config") {}

  function globalConfigFile() {
    const candidates = ["codeforge.jsonc", "codeforge.json", "config.json"].map((file) =>
      ...
      Effect.gen(function* () {
        ...
        const loadConfig = Effect.fnUntraced(function* (
          ...
          const normalized = (() => {
            ...
            delete copy.tui
            log.warn("tui keys in codeforge config are deprecated; move them to tui.json", { path: source })
            return copy
          })()
          ...
          if (parsed.success) {
            if (!parsed.data.$schema && isFile) {
              parsed.data.$schema = "https://YOUR_DOMAIN.ai/config.json"
        ...
        const loadGlobal = Effect.fnUntraced(function* () {
          let result: Info = pipe(
            ...
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "codeforge.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "codeforge.jsonc"))),
          )
            ...
            yield* Effect.promise(() =>
              import(pathToFileURL(legacy).href, { with: { type: "toml" } })
                .then(async (mod) => {
                  ...
                  result["$schema"] = "https://YOUR_DOMAIN.ai/config.json"
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          for (const [key, value] of Object.entries(auth)) {
            if (value.type === "wellknown") {
              ...
              log.debug("fetching remote config", { url: `${url}/.well-known/codeforge` })
              const response = yield* Effect.promise(() => fetch(`${url}/.well-known/codeforge`))
              if (!response.ok) {
                throw new Error(`failed to fetch remote config from ${url}: ${response.status}`)
              }
              const wellknown = (yield* Effect.promise(() => response.json())) as any
              const remoteConfig = wellknown.config ?? {}
              if (!remoteConfig.$schema) remoteConfig.$schema = "https://YOUR_DOMAIN.ai/config.json"
              result = mergeConfigConcatArrays(
                result,
                yield* loadConfig(JSON.stringify(remoteConfig), {
                  dir: path.dirname(`${url}/.well-known/codeforge`),
                  source: `${url}/.well-known/codeforge`,
                }),
              )
          ...
          if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("codeforge", ctx.directory, ctx.worktree),
            )) {
          ...
          for (const dir of unique(directories)) {
            if (dir.endsWith(".codeforge") || dir === Flag.OPENCODE_CONFIG_DIR) {
              for (const file of ["codeforge.jsonc", "codeforge.json"]) {
