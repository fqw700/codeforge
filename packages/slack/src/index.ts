...
import { createOpencode, type ToolPart } from "@codeforge-ai/sdk"
...
console.log("🚀 Starting codeforge server...")
const codeforge = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await codeforge.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new codeforge session...")
    const { client, server } = codeforge
    ...
    console.log("✅ Created codeforge session:", createResult.data.id)
  ...
  console.log("📝 Sending to codeforge:", message.text)
