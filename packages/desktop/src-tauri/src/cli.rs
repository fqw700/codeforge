...
const CLI_INSTALL_DIR: &str = ".codeforge/bin";
const CLI_BINARY_NAME: &str = "codeforge";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("codeforge-install.sh");
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.codeforge/bin/codeforge\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://YOUR_DOMAIN.ai/install | bash -s -- --version {} --no-modify-path",
...
pub fn serve(
    ...
    let envs = [
        ("OPENCODE_SERVER_USERNAME", "codeforge".to_string()),
    ...
    .expect("Failed to spawn codeforge");
