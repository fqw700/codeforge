...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("codeforge-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("codeforge_lib=debug,codeforge_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("codeforge_lib=info,codeforge_desktop=info,sidecar=info")
        }
    });
