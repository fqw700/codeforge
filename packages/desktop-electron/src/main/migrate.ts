...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.codeforge.desktop.dev",
  beta: "ai.codeforge.desktop.beta",
  prod: "ai.codeforge.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.codeforge.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `codeforge.settings.dat` is special: it maps to the `codeforge.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"codeforge.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // codeforge.settings.dat → the electron settings store ("codeforge.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "codeforge.global.dat").
  const storeName = filename === "codeforge.settings.dat" ? "codeforge.settings" : filename
