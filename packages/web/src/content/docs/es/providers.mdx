---
title: Proveedores
description: Usando cualquier proveedor LLM en CodeForge.
---

import config from "../../../../config.mjs"
export const console = config.console

CodeForge usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración CodeForge.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu CodeForge
configuración.
...
CodeForge Zen es una lista de modelos proporcionados por el equipo CodeForge que han sido
probado y verificado para funcionar bien con CodeForge. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con CodeForge Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione codeforge y diríjase a [codeforge.ai/auth](https://YOUR_DOMAIN.ai/auth).
...
Funciona como cualquier otro proveedor en CodeForge y su uso es completamente opcional.

---

## CodeForge Go

CodeForge Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de CodeForge que han sido
probado y verificado para funcionar bien con CodeForge.

1. Ejecute el comando `/connect` en TUI, seleccione `CodeForge Go` y diríjase a [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen).
...
Para usar Amazon Bedrock con CodeForge:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta codeforge:
   ...
   Para una configuración persistente o específica del proyecto, utilice `codeforge.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en CodeForge no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que codeforge funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración codeforge.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `CodeForge`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación codeforge.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
CodeForge utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear CodeForge
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `codeforge.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [codeforge-gitlab-auth](https://www.npmjs.com/package/codeforge-gitlab-auth).

##### Configuración

Personalizar a través de `codeforge.json`:
...
Para utilizar su suscripción GitHub Copilot con codeforge:
...
Para utilizar Google Vertex AI con CodeForge:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta codeforge.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de codeforge, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [codeforge-helicone-session](https://github.com/H2Shami/codeforge-helicone-session) para registrar automáticamente cada conversación CodeForge como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación CodeForge enumerada como una sesión separada.
...
Puede configurar codeforge para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar codeforge para usar modelos locales a través de LM Studio.
...
Puedes configurar codeforge para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para CodeForge. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/codeforge) para obtener más detalles.
:::
...
Para usar Ollama Cloud con CodeForge:
...
3. Copie la clave API para usarla en CodeForge.
...
6. **Importante**: Antes de usar modelos en la nube en CodeForge, debe obtener la información del modelo localmente:
...
CodeForge Zen es una lista de modelos probados y verificados proporcionada por el equipo CodeForge. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>CodeForge Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **CodeForge Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave CodeForge API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración codeforge.
...
5. También puedes personalizarlos a través de tu configuración codeforge. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con CodeForge:
...
También puede personalizar modelos a través de su configuración codeforge. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con codeforge. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `codeforge.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a CodeForge comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `codeforge auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración codeforge y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración codeforge.
