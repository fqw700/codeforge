...
Las habilidades del agente permiten a CodeForge descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
CodeForge busca estas ubicaciones:

- Configuración del proyecto: `.codeforge/skills/<name>/SKILL.md`
- Configuración global: `~/.config/codeforge/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, CodeForge sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.codeforge/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/codeforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.codeforge/skills/git-release/SKILL.md` así:
...
CodeForge enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `codeforge.json`:
...
**Para agentes integrados** (en `codeforge.json`):
