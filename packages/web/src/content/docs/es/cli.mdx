---
title: CLI
description: Opciones y comandos de la CLI de CodeForge.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de CodeForge de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
codeforge
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con CodeForge mediante programación.
...
Inicie la interfaz de usuario del terminal CodeForge.
...
La CLI de CodeForge también tiene los siguientes comandos.

---

### agent

Administrar agentes para CodeForge.
...
Conecte una terminal a un servidor backend CodeForge que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
codeforge attach [url]
```

Esto permite usar TUI con un backend remoto CodeForge. Por ejemplo:
...
CodeForge funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `codeforge auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/codeforge/auth.json`.

```bash
codeforge auth login
```

Cuando se inicia CodeForge, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en CodeForge.
...
Ejecute codeforge en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `codeforge serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| `--attach`   |       | Adjuntar a un servidor codeforge en ejecución (por ejemplo, http://localhost:4096)   |
...
Inicie un servidor CodeForge sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
codeforge serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad codeforge sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `codeforge`).
...
Administrar CodeForge sesiones.
...
Enumere todas las sesiones CodeForge.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones CodeForge.
...
Importe datos de sesión desde un archivo JSON o una URL compartida CodeForge.

```bash
codeforge import <file>
```

Puede importar desde un archivo local o una URL compartida CodeForge.
...
Inicie un servidor CodeForge sin cabeza con una interfaz web.

```bash
codeforge web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a CodeForge a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `codeforge`).
...
Desinstale CodeForge y elimine todos los archivos relacionados.
...
Actualiza codeforge a la última versión o a una versión específica.
...
La CLI de CodeForge toma las siguientes banderas globales.
...
CodeForge se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `codeforge`) |
