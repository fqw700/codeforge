...
Con CodeForge puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, CodeForge usa nuestro propio tema `codeforge`.
...
CodeForge viene con varios temas integrados.
...
- Quiere que CodeForge coincida con la apariencia de su terminal
...
CodeForge admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/codeforge/themes/*.json` o `$XDG_CONFIG_HOME/codeforge/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.codeforge/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.codeforge/themes/*.json`
