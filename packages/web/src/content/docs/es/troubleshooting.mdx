...
Para depurar problemas con CodeForge, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\codeforge\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `codeforge --log-level DEBUG`.

---

## Almacenamiento

codeforge almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/codeforge/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\codeforge`
...
CodeForge Desktop ejecuta un servidor CodeForge local (el sidecar `codeforge-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `CodeForge` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/codeforge/codeforge.jsonc` (o `~/.config/codeforge/codeforge.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/codeforge/codeforge.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\codeforge\codeforge.jsonc`
...
CodeForge también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/codeforge/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\codeforge\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que CodeForge pueda reconstruirlo.

1. Salga de CodeForge Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/codeforge`
- **Linux**: eliminar `~/.cache/codeforge` (o ejecutar `rm -rf ~/.cache/codeforge`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\codeforge`

3. Reinicie el escritorio CodeForge.

---

### Solucionar problemas de conexión del servidor

CodeForge Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `codeforge.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio CodeForge requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de CodeForge.

---

### Notificaciones que no se muestran

CodeForge Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para CodeForge en la configuración de su sistema operativo, y
...
1. Salga del escritorio CodeForge.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio CodeForge):

- `codeforge.settings.dat` (URL del servidor predeterminado de escritorio)
- `codeforge.global.dat` y `codeforge.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con CodeForge:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/YOUR_ORG/codeforge/issues**](https://github.com/YOUR_ORG/codeforge/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**codeforge.ai/discord**](https://YOUR_DOMAIN.ai/discord)
...
3. Asegúrese de tener la última versión con `codeforge upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `codeforge models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/codeforge
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\codeforge`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. codeforge instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\codeforge`

2. Reinicie codeforge para reinstalar los paquetes del proveedor más recientes.

Esto obligará a codeforge a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
codeforge detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
