...
Puede agregar herramientas externas a CodeForge utilizando el _Model Context Protocol_ o MCP. CodeForge admite servidores locales y remotos.
...
Puede definir servidores MCP en su [CodeForge Config](https://YOUR_DOMAIN.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/codeforge`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
CodeForge maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, CodeForge:
...
Si el servidor requiere autenticación, CodeForge le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `codeforge mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, CodeForge almacenará los tokens de forma segura en `~/.local/share/codeforge/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en CodeForge, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración CodeForge como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar CodeForge a su cuenta Sentry.
