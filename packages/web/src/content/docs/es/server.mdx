---
title: Servidor
description: Interactuar con el servidor codeforge a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `codeforge serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente codeforge puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `codeforge.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `codeforge`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `codeforge serve` como a `codeforge web`.
...
Cuando ejecuta `codeforge`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor codeforge para interactuar con codeforge mediante programación.
:::

Esta arquitectura permite que codeforge admita múltiples clientes y le permite interactuar con codeforge mediante programación.

Puede ejecutar `codeforge serve` para iniciar un servidor independiente. Si tienes el
codeforge TUI ejecutándose, `codeforge serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos CodeForge [IDE](/docs/ide).
...
El servidor codeforge expone las siguientes API.
