---
title: Complementos
description: Escriba sus propios complementos para extender CodeForge.
---

Los complementos le permiten extender CodeForge al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de CodeForge.
...
- `.codeforge/plugins/` - Complementos a nivel de proyecto
- `~/.config/codeforge/plugins/` - Complementos globales
...
```json title="codeforge.json"
{
  ...
  "plugin": ["codeforge-helicone-session", "codeforge-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/codeforge/node_modules/`.
...
1. Configuración global (`~/.config/codeforge/codeforge.json`)
2. Configuración del proyecto (`codeforge.json`)
3. Directorio global de complementos (`~/.config/codeforge/plugins/`)
4. Directorio de complementos del proyecto (`.codeforge/plugins/`)
...
```json title=".codeforge/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
CodeForge ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".codeforge/plugins/my-plugin.ts"
...
```js title=".codeforge/plugins/example.js"
...
- `client`: Un cliente SDK codeforge para interactuar con la IA.
...
import type { Plugin } from "@codeforge-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar codeforge.
...
```js title=".codeforge/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "codeforge"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio CodeForge, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que codeforge lea archivos `.env`:

```javascript title=".codeforge/plugins/env-protection.js"
...
```javascript title=".codeforge/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a codeforge:

```ts title=".codeforge/plugins/custom-tools.ts"
import { type Plugin, tool } from "@codeforge-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que codeforge puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para codeforge junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://YOUR_DOMAIN.ai/docs/sdk) para obtener más detalles.
...
```ts title=".codeforge/plugins/compaction.ts"
import type { Plugin } from "@codeforge-ai/plugin"
...
```ts title=".codeforge/plugins/custom-compaction.ts"
import type { Plugin } from "@codeforge-ai/plugin"
