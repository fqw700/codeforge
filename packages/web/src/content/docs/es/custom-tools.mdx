...
description: Cree herramientas que LLM pueda llamar en codeforge.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de codeforge como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.codeforge/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/codeforge/tools/`.
...
```ts title=".codeforge/tools/database.ts" {1}
...
```ts title=".codeforge/tools/math.ts"
import { tool } from "@codeforge-ai/plugin"
...
```ts title=".codeforge/tools/bash.ts"
import { tool } from "@codeforge-ai/plugin"
...
```ts title=".codeforge/tools/project.ts" {8}
import { tool } from "@codeforge-ai/plugin"
...
```python title=".codeforge/tools/add.py"
import sys
...
```ts title=".codeforge/tools/python-add.ts" {10}
import { tool } from "@codeforge-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".codeforge/tools/add.py")
