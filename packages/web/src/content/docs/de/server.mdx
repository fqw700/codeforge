---
title: Server
description: Nutze den codeforge-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `codeforge serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein codeforge-Client nutzen kann.
...
Der Benutzername ist standardmaessig `codeforge`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `codeforge serve` und `codeforge web`.
...
Wenn du `codeforge` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den codeforge-Server, um codeforge programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `codeforge serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `codeforge serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die CodeForge-[IDE](/docs/ide)-Plugins.
...
Der codeforge-Server stellt folgende APIs bereit.
