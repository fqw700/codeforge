---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in CodeForge.
---

import config from "../../../../config.mjs"
export const console = config.console

CodeForge verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer CodeForge-Konfiguration.
...
in `~/.local/share/codeforge/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem CodeForge anpassen
config.
...
CodeForge Zen ist eine Liste von Modellen, die vom CodeForge-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit CodeForge funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit CodeForge Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie CodeForge aus und gehen Sie zu [codeforge.ai/auth](https://YOUR_DOMAIN.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in CodeForge und ist völlig optional.

---

## CodeForge Go

CodeForge Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom CodeForge-Team getestet und verifiziert wurden, dass sie gut mit CodeForge funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `CodeForge Go` und gehen Sie zu [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen).
...
So verwenden Sie Amazon Bedrock mit CodeForge:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie CodeForge ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `codeforge.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in CodeForge wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit CodeForge ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre CodeForge-Konfiguration hinzufügen.
...
um das Token im CodeForge-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
CodeForge verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von CodeForge
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`codeforge.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [codeforge-gitlab-auth](https://www.npmjs.com/package/codeforge-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `codeforge.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit CodeForge:
...
So verwenden Sie Google Vertex AI mit CodeForge:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie CodeForge ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über CodeForge konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [codeforge-helicone-session](https://github.com/H2Shami/codeforge-helicone-session)-Plugin, um jede CodeForge-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede CodeForge-Konversation als separate Sitzung aufgeführt.
...
Sie können CodeForge für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können CodeForge über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können CodeForge über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für CodeForge konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/codeforge).
:::
...
So verwenden Sie Ollama Cloud mit CodeForge:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in CodeForge.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in CodeForge verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
CodeForge Zen ist eine Liste getesteter und verifizierter Modelle, die vom CodeForge-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>CodeForge Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **CodeForge Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren CodeForge API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre CodeForge-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre CodeForge-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit CodeForge:
...
Sie können Modelle auch über Ihre CodeForge-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit CodeForge verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `codeforge.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann CodeForge erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `codeforge auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die CodeForge-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer CodeForge-Konfiguration übereinstimmt.
