---
title: Regeln
description: Lege eigene Anweisungen fuer codeforge fest.
---

Du kannst codeforge ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in codeforge den Befehl `/init` aus.
...
So kann codeforge sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht codeforge sie zu erweitern.
...
codeforge kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/codeforge/AGENTS.md` speichern.
Diese gelten in allen codeforge-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt CodeForge dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/codeforge/AGENTS.md` existiert)
...
Beim Start sucht codeforge in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/codeforge/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `codeforge.json` oder in `~/.config/codeforge/codeforge.json` angeben.
...
codeforge parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit codeforge.json

Empfohlen ist das `instructions`-Feld in der `codeforge.json`:
...
Du kannst codeforge explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `codeforge.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
