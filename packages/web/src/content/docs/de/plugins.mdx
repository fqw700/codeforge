---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um CodeForge zu erweitern.
---

Mit Plugins können Sie CodeForge erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von CodeForge zu ändern.
...
- `.codeforge/plugins/` – Plugins auf Projektebene
- `~/.config/codeforge/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/codeforge/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/codeforge/codeforge.json`)
2. Projektkonfiguration (`codeforge.json`)
3. Globales Plugin-Verzeichnis (`~/.config/codeforge/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.codeforge/plugins/`)
...
CodeForge führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein CodeForge-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie CodeForge erweitern können.
...
:::note
Wenn Sie die CodeForge-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass CodeForge `.env`-Dateien liest:
...
Plugins können CodeForge auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das CodeForge aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für CodeForge zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://YOUR_DOMAIN.ai/docs/sdk).
