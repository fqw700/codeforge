...
Mit CodeForge kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet CodeForge das `codeforge`-Theme.
...
CodeForge liefert mehrere eingebaute Themes mit.
...
- CodeForge optisch ans Terminal anpassen wollen
...
CodeForge unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/codeforge/themes/*.json` oder `$XDG_CONFIG_HOME/codeforge/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.codeforge/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.codeforge/themes/*.json`
