...
Wenn CodeForge Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codeforge\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `codeforge --log-level DEBUG`.

---

## Speicher

codeforge speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/codeforge/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codeforge`
...
CodeForge Desktop startet im Hintergrund einen lokalen CodeForge-Server (`codeforge-cli`-Sidecar).
...
- Nur macOS: `CodeForge`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/codeforge/codeforge.jsonc` (oder `~/.config/codeforge/codeforge.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/codeforge/codeforge.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codeforge\codeforge.jsonc`
...
CodeForge kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/codeforge/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codeforge\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit CodeForge ihn neu aufbauen kann.

1. Beende CodeForge Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/codeforge`
- **Linux**: loesche `~/.cache/codeforge` (oder fuehre aus: `rm -rf ~/.cache/codeforge`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\codeforge`

3. Starte CodeForge Desktop neu.

---

### Server-Verbindungsprobleme beheben

CodeForge Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `codeforge.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt CodeForge Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit CodeForge-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

CodeForge Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer CodeForge in den OS-Einstellungen aktiviert sind, und
...
1. Beende CodeForge Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von CodeForge Desktop):

- `codeforge.settings.dat` (desktop default server URL)
- `codeforge.global.dat` und `codeforge.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit CodeForge hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `codeforge models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\codeforge`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. codeforge installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\codeforge`

2. Starte codeforge neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt codeforge, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
codeforge erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
