...
CodeForge Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
CodeForge Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in CodeForge. Du abonnierst CodeForge Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
CodeForge zu verwenden.
...
:::tip
Wir haben eine ausgewählte Gruppe von Modellen und Providern getestet, die gut mit CodeForge funktionieren.
:::
...
CodeForge Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

CodeForge Go funktioniert wie jeder andere Provider in CodeForge.

1. Du meldest dich bei **<a href={console}>CodeForge Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `CodeForge Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann CodeForge Go abonnieren.
:::
...
CodeForge Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner CodeForge Config verwendet das Format `codeforge-go/<model-id>`. Für Kimi K2.5 würdest du beispielsweise `codeforge-go/kimi-k2.5` in deiner Config verwenden.
...
Wir haben CodeForge Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit CodeForge zu nutzen.
