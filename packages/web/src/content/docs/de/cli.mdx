---
title: CLI
description: CodeForge CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der CodeForge CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
codeforge
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit CodeForge interagieren.
...
Starten Sie die Terminalbenutzeroberfläche CodeForge.
...
Der CodeForge CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für CodeForge verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden CodeForge-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
codeforge attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend CodeForge. Zum Beispiel:
...
CodeForge wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `codeforge auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/codeforge/auth.json` gespeichert.

```bash
codeforge auth login
```

Wenn CodeForge startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in CodeForge sehen möchten.
...
Führen Sie CodeForge im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `codeforge serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| `--attach`   |      | An einen laufenden CodeForge-Server anschließen (z.B. http://localhost:4096)                         |
...
Starten Sie einen Headless-CodeForge-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
codeforge serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf CodeForge-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `codeforge`).
...
Listen Sie alle CodeForge-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre CodeForge-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer CodeForge-Freigabe-URL.

```bash
codeforge import <file>
```

Sie können aus einer lokalen Datei oder einer CodeForge-Freigabe-URL importieren.
...
Starten Sie einen Headless-CodeForge-Server mit einer Weboberfläche.

```bash
codeforge web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf CodeForge zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `codeforge`).
...
Deinstallieren Sie CodeForge und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert CodeForge auf die neueste Version oder eine bestimmte Version.
...
Der CodeForge CLI akzeptiert die folgenden globalen Flags.
...
CodeForge kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `codeforge`) |
