...
Agent Skills erlauben CodeForge, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
CodeForge sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft CodeForge vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.codeforge/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/codeforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.codeforge/skills/git-release/SKILL.md` zum Beispiel so:
...
CodeForge listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `codeforge.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `codeforge.json`):
