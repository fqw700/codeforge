---
title: Zen
description: codeforge tarafından sağlanan seçilmiş modeller listesi.
---
...
CodeForge Zen, codeforge ekibi tarafından test edilip doğrulanmış modellerin bir listesidir.

:::note
CodeForge Zen şu anda beta aşamasındadır.
:::

Zen, codeforge'daki diğer sağlayıcılar gibi çalışır. CodeForge Zen'e giriş yapar ve API anahtarınızı alırsınız. Tamamen isteğe bağlıdır ve codeforge kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
codeforge ile iyi çalışan seçkin bir grup model ve sağlayıcıyı test ettik.
:::
...
CodeForge Zen, bu modellere erişmenizi sağlayan bir AI ağ geçididir.

---

## Nasıl çalışır

CodeForge Zen, codeforge'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>CodeForge Zen</a>**'de oturum açın, fatura ayrıntılarınızı ekleyin ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, CodeForge Zen'i seçin ve API anahtarınızı yapıştırın.
...
codeforge yapılandırmanızdaki [model kimliği](/docs/config/#models) `codeforge/<model-id>` biçimini kullanır. Örneğin, GPT 5.2 Codex için yapılandırmanızda `codeforge/gpt-5.2-codex` kullanırsınız.
...
- MiniMax M2.5 Free, sınırlı bir süre için CodeForge'da ücretsizdir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
- Big Pickle, sınırlı bir süre için CodeForge'da ücretsiz olan gizli bir modeldir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
...
CodeForge Zen'i şu amaçlarla oluşturduk:
...
4. Başka bir kodlama ajanıyla kullanmanıza izin vererek **kilitlenmeyi önlemek**. Ve her zaman CodeForge ile başka bir sağlayıcıyı kullanmanıza izin vermek.
