---
title: CLI
description: codeforge CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

codeforge CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
codeforge
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, codeforge ile programlı olarak etkileşim kurmanıza olanak tanır.
...
codeforge terminal kullanıcı arayüzünü başlatır.
...
codeforge CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

codeforge için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir codeforge arka uç sunucusuna bir terminal ekleyin.

```bash
codeforge attach [url]
```

Bu, TUI öğesinin uzak bir codeforge arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
codeforge, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `codeforge auth login` kullanabilirsiniz. Bu `~/.local/share/codeforge/auth.json`'de saklanıyor.

```bash
codeforge auth login
```

codeforge başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları codeforge'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek codeforge'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `codeforge serve` örneğine de ekleyebilirsiniz:
...
| `--attach`   |      | Çalışan bir codeforge sunucusuna ekleyin (ör. http://localhost:4096)               |
...
API erişimi için headless bir codeforge sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
codeforge serve
```

Bu, TUI arayüzü olmadan codeforge işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `codeforge` şeklindedir).
...
codeforge oturumlarını yönetin.
...
Tüm codeforge oturumlarını listeleyin.
...
codeforge oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya codeforge paylaşımından URL oturum verilerini içe aktarın.

```bash
codeforge import <file>
```

Yerel bir dosyadan veya codeforge paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir codeforge sunucusu başlatın.

```bash
codeforge web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla codeforge'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `codeforge` şeklindedir).
...
codeforge'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
codeforge'u en son sürüme veya belirli bir sürüme günceller.
...
codeforge CLI aşağıdaki global bayrakları destekler.
...
codeforge ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `codeforge`) |
