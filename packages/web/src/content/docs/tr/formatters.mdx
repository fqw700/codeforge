...
description: codeforge dile özgü biçimlendiriciler kullanır.
---

codeforge, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

codeforge, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, codeforge bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

codeforge bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri codeforge yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
