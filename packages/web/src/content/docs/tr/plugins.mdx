---
title: Eklentiler
description: CodeForge'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek CodeForge'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya CodeForge'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.codeforge/plugins/` - Proje düzeyinde eklentiler
- `~/.config/codeforge/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/codeforge/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/codeforge/codeforge.json`)
2. Project config (`codeforge.json`)
3. Global eklenti dizini (`~/.config/codeforge/plugins/`)
4. Proje eklenti dizini (`.codeforge/plugins/`)
...
CodeForge bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir CodeForge SDK istemcisi.
...
CodeForge'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
CodeForge masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
CodeForge'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca CodeForge'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, CodeForge'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra CodeForge'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://YOUR_DOMAIN.ai/docs/sdk) bakın.
