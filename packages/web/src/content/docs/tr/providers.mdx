---
title: Sağlayıcılar
description: codeforge'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

codeforge, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı codeforge yapılandırmanızda yapılandırın.
...
`~/.local/share/codeforge/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları codeforge'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
CodeForge Zen, codeforge ekibi tarafından sağlanan modellerin bir listesidir.
codeforge ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz CodeForge Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, codeforge'u seçin ve [codeforge.ai/auth](https://YOUR_DOMAIN.ai/auth)'ye gidin.
...
codeforge'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## CodeForge Go

CodeForge Go, codeforge ile iyi çalıştığı test edilmiş ve doğrulanmış, codeforge ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `CodeForge Go`'yu seçin ve [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen) adresine gidin.
...
Amazon Bedrock'u codeforge ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   codeforge'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `codeforge.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi codeforge'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   codeforge'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in codeforge auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
codeforge, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. codeforge'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`codeforge.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [codeforge-gitlab-auth](https://www.npmjs.com/package/codeforge-gitlab-auth) ana sayfasında.

##### Yapılandırma

`codeforge.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi codeforge'la kullanmak için:
...
Google Vertex AI'yi codeforge ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları codeforge çalıştırılırken ayarlayın.
...
Helicone'un codeforge aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her codeforge görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [codeforge-helicone-session](https://github.com/H2Shami/codeforge-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her codeforge görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
codeforge'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla codeforge'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla codeforge'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini codeforge için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/codeforge)'a bakın.
:::
...
Ollama Cloud'u codeforge ile kullanmak için:
...
3. codeforge'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: codeforge'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
CodeForge Zen, codeforge ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>CodeForge Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **CodeForge Zen**'i arayın.

   ```txt
   /connect
   ```

3. codeforge API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   codeforge yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca codeforge yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri codeforge yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı codeforge ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `codeforge.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları codeforge'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `codeforge auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için codeforge yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, codeforge yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
