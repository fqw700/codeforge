...
_Model Bağlam Protokolü_ veya MCP kullanarak codeforge'a harici araçlar ekleyebilirsiniz. codeforge hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [codeforge Config](https://YOUR_DOMAIN.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/codeforge` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
codeforge, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde codeforge şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa codeforge, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `codeforge mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından codeforge, belirteçleri `~/.local/share/codeforge/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra codeforge'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi codeforge yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve codeforge'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
