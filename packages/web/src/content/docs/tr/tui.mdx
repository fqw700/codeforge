---
title: TUI
description: CodeForge terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodeForge, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

CodeForge'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
CodeForge TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
CodeForge'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
CodeForge'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `codeforge.json` dosyasından ayrıdır.
