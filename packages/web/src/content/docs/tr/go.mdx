...
CodeForge Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
CodeForge Go şu anda beta olarak mevcut.
:::

Go, CodeForge'daki diğer sağlayıcılar gibi çalışır. CodeForge Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve CodeForge'u kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
CodeForge ile iyi çalışan seçili bir model ve sağlayıcı grubunu test ettik.
:::
...
CodeForge Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

CodeForge Go, CodeForge'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>CodeForge Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `CodeForge Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye CodeForge Go'ya abone olabilir.
:::
...
CodeForge Go aşağıdaki limitleri içerir:
...
CodeForge yapılandırmanızdaki [model id](/docs/config/#models) formatı `codeforge-go/<model-id>` şeklindedir. Örneğin, Kimi K2.5 için yapılandırmanızda `codeforge-go/kimi-k2.5` kullanmalısınız.
...
CodeForge Go'yu şu amaçlarla oluşturduk:
...
4. CodeForge ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
