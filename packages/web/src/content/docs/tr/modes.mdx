...
:::caution
Modlar artık codeforge yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

codeforge'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya codeforge yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
codeforge iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.codeforge/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`codeforge.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, codeforge modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global codeforge yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.codeforge/modes/` veya genel modlar için `~/.config/codeforge/modes/`'de mod dosyaları oluşturun:
