...
codeforge'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
codeforge, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
codeforge, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

codeforge iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `codeforge.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse codeforge modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global codeforge yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
codeforge yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [CodeForge Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `codeforge/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `codeforge models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/YOUR_ORG/codeforge).
:::
