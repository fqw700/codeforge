...
codeforge ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak codeforge kendi `codeforge` temasını kullanır.
...
codeforge birden fazla yerleşik temayla gelir.
...
- codeforge'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
codeforge, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/codeforge/themes/*.json` veya `$XDG_CONFIG_HOME/codeforge/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.codeforge/themes/*.json`
4. **Geçerli çalışma dizini** - `./.codeforge/themes/*.json`
