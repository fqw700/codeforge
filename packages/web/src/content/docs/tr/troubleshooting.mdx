...
codeforge ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\codeforge\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `codeforge --log-level DEBUG`.

---

## Depolama

codeforge, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/codeforge/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\codeforge` yapistirin
...
codeforge Desktop arka planda yerel bir codeforge sunucusu (`codeforge-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `codeforge` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/codeforge/codeforge.jsonc` (veya `~/.config/codeforge/codeforge.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/codeforge/codeforge.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\codeforge\codeforge.jsonc` yapistirin
...
codeforge diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/codeforge/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\codeforge\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), codeforge'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. codeforge Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/codeforge`
- **Linux**: `~/.cache/codeforge` dizinini silin (veya `rm -rf ~/.cache/codeforge` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\codeforge` yapistirin

3. codeforge Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

codeforge Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`codeforge.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta codeforge Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, codeforge ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

codeforge Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda codeforge icin bildirimler etkinse
...
1. codeforge Desktop'u kapatin
2. Su dosyalari bulun ve silin (codeforge Desktop uygulama veri dizininde yer alirlar):

- `codeforge.settings.dat` (desktop varsayilan sunucu URL'si)
- `codeforge.global.dat` ve `codeforge.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
codeforge ile ilgili bir sorun yasiyorsaniz:
...
Erisiminiz olan modelleri gormek icin `codeforge models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\codeforge`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. codeforge, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\codeforge`

2. En guncel provider paketlerini yeniden kurmak icin codeforge'u yeniden baslatin

Bu, codeforge'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
codeforge, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
