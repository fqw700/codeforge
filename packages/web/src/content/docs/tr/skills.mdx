...
Ajan becerileri, codeforge'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
codeforge su konumlari tarar:

- Proje konfigurasyonu: `.codeforge/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/codeforge/skills/<name>/SKILL.md`
...
Proje yerel yollarinda codeforge, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.codeforge/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/codeforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.codeforge/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
codeforge, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`codeforge.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`codeforge.json` icinde):
