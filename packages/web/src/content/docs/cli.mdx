---
title: CLI
description: CodeForge CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The CodeForge CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
codeforge
```

But it also accepts commands as documented on this page. This allows you to interact with CodeForge programmatically.
...
Start the CodeForge terminal user interface.
...
The CodeForge CLI also has the following commands.

---

### agent

Manage agents for CodeForge.
...
Attach a terminal to an already running CodeForge backend server started via `serve` or `web` commands.

```bash
codeforge attach [url]
```

This allows using the TUI with a remote CodeForge backend. For example:
...
CodeForge is powered by the provider list at [Models.dev](https://models.dev), so you can use `codeforge auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/codeforge/auth.json`.

```bash
codeforge auth login
```

When CodeForge starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in CodeForge.
...
Run codeforge in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `codeforge serve` instance to avoid MCP server cold boot times on every run:
...
| `--attach`   |       | Attach to a running codeforge server (e.g., http://localhost:4096)       |
...
Start a headless CodeForge server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
codeforge serve
```

This starts an HTTP server that provides API access to codeforge functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `codeforge`).
...
Show token usage and cost statistics for your CodeForge sessions.
...
Import session data from a JSON file or CodeForge share URL.

```bash
codeforge import <file>
```

You can import from a local file or an CodeForge share URL.
...
Start a headless CodeForge server with a web interface.

```bash
codeforge web
```

This starts an HTTP server and opens a web browser to access CodeForge through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `codeforge`).
...
Uninstall CodeForge and remove all related files.
...
Updates codeforge to the latest version or a specific version.
...
The codeforge CLI takes the following global flags.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `codeforge`) |
