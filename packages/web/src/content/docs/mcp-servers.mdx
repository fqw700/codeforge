...
You can add external tools to CodeForge using the _Model Context Protocol_, or MCP. CodeForge supports both local and remote servers.
...
You can define MCP servers in your [CodeForge Config](https://YOUR_DOMAIN.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/codeforge` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
CodeForge automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, CodeForge will:
...
If the server requires authentication, CodeForge will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `codeforge mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, CodeForge will store the tokens securely in `~/.local/share/codeforge/mcp-auth.json`.
...
Your MCPs are available as tools in CodeForge, alongside built-in tools. So you can manage them through the CodeForge config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect CodeForge to your Sentry account.
