...
CodeForge Go est un abonnement à bas coût — **5 $ pour votre premier mois**, puis **10 $/mois** — qui vous donne un accès fiable aux modèles de codage ouverts populaires.

:::note
CodeForge Go est actuellement en version bêta.
:::

Go fonctionne comme n'importe quel autre fournisseur dans CodeForge. Vous vous abonnez à CodeForge Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser CodeForge.
...
CodeForge Go vous donne accès à ces modèles pour **5 $ pour votre premier mois**, puis **10 $/mois**.

---

## Comment ça marche

CodeForge Go fonctionne comme n'importe quel autre fournisseur dans CodeForge.

1. Vous vous connectez à **<a href={console}>CodeForge Zen</a>**, vous vous abonnez à Go et copiez votre clé d'API.
2. Vous exécutez la commande `/connect` dans la TUI, sélectionnez `CodeForge Go` et collez votre clé d'API.
3. Exécutez `/models` dans la TUI pour voir la liste des modèles disponibles via Go.

:::note
Un seul membre par espace de travail peut s'abonner à CodeForge Go.
:::
...
CodeForge Go inclut les limites suivantes :
...
| ------------ | ------------ | ------------------------------------------------ | --------------------------- |
| GLM-5        | glm-5        | `https://YOUR_DOMAIN.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| Kimi K2.5    | kimi-k2.5    | `https://YOUR_DOMAIN.ai/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| MiniMax M2.7 | minimax-m2.7 | `https://YOUR_DOMAIN.ai/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |
| MiniMax M2.5 | minimax-m2.5 | `https://YOUR_DOMAIN.ai/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |

L'[ID de modèle](/docs/config/#models) dans votre configuration CodeForge utilise le format `codeforge-go/<model-id>`. Par exemple, pour Kimi K2.5, vous utiliseriez `codeforge-go/kimi-k2.5` dans votre configuration.
...
Nous avons créé CodeForge Go pour :
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec CodeForge également.
