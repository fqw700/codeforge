---
title: Plugins
description: Écrivez vos propres plugins pour étendre CodeForge.
---

Les plugins vous permettent d'étendre CodeForge en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de CodeForge.
...
- `.codeforge/plugins/` - Plugins au niveau du projet
- `~/.config/codeforge/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/codeforge/node_modules/`.
...
1. Configuration globale (`~/.config/codeforge/codeforge.json`)
2. Configuration du projet (`codeforge.json`)
3. Répertoire global des plugins (`~/.config/codeforge/plugins/`)
4. Répertoire des plugins du projet (`.codeforge/plugins/`)
...
CodeForge exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK codeforge pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre codeforge.
...
:::note
Si vous utilisez l'application de bureau CodeForge, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez codeforge de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à codeforge :
...
L'assistant `tool` crée un outil personnalisé que codeforge peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour codeforge aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://YOUR_DOMAIN.ai/docs/sdk) pour plus de détails.
