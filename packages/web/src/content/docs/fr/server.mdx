---
title: Serveur
description: Interagissez avec le serveur codeforge via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `codeforge serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client codeforge peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `codeforge.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `codeforge`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `codeforge serve` et à `codeforge web`.
...
Lorsque vous exécutez `codeforge`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur codeforge pour interagir avec codeforge par programmation.
:::

Cette architecture permet à codeforge de prendre en charge plusieurs clients et vous permet d'interagir avec codeforge par programme.

Vous pouvez exécuter `codeforge serve` pour démarrer un serveur autonome. Si vous avez le
codeforge TUI en cours d'exécution, `codeforge serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins CodeForge [IDE](/docs/ide).
...
Le serveur codeforge expose les API suivantes.
