...
Il existe deux types d'agents dans CodeForge ; agents primaires et sous-agents.
...
CodeForge est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
CodeForge est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

CodeForge est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `codeforge.json` :
...
- Global : `~/.config/codeforge/agents/`
- Par projet : `.codeforge/agents/`

```markdown title="~/.config/codeforge/agents/review.md"
---
...
Si aucune température n'est spécifiée, CodeForge utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale CodeForge et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration CodeForge utilise le format `provider/model-id`. Par exemple, si vous utilisez [CodeForge Zen](/docs/zen), vous utiliserez `codeforge/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="codeforge.json" {3-6,9-12}
{
  "$schema": "https://YOUR_DOMAIN.ai/config.json",
  "tools": {
    "write": true,
...
```json title="codeforge.json" {3-5,8-10}
{
  "$schema": "https://YOUR_DOMAIN.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `codeforge models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/YOUR_ORG/codeforge).
:::
...
```markdown title="~/.config/codeforge/agents/docs-writer.md"
---
...
```markdown title="~/.config/codeforge/agents/security-auditor.md"
---
