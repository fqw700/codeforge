...
Avec CodeForge, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, CodeForge utilise notre propre thème `codeforge`.
...
CodeForge est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que CodeForge corresponde à l'apparence de leur terminal
...
CodeForge prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/codeforge/themes/*.json` ou `$XDG_CONFIG_HOME/codeforge/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.codeforge/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.codeforge/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/codeforge/themes
vim ~/.config/codeforge/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .codeforge/themes
vim .codeforge/themes/my-theme.json
```
...
{
  "$schema": "https://YOUR_DOMAIN.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
