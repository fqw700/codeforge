...
مع CodeForge يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم CodeForge سمتنا `codeforge`.
...
يأتي CodeForge مع عدة سمات مدمجة.
...
- يريدون أن يطابق CodeForge مظهر terminal لديهم
...
يدعم CodeForge نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/codeforge/themes/*.json` أو `$XDG_CONFIG_HOME/codeforge/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.codeforge/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.codeforge/themes/*.json`
