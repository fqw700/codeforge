---
title: الخادم
description: تفاعل مع خادم codeforge عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `codeforge serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل codeforge استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `codeforge.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `codeforge`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `codeforge serve` و `codeforge web`.
...
عند تشغيل `codeforge` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم codeforge للتفاعل مع codeforge برمجيا.
:::

تتيح هذه البنية لـ codeforge دعم عدة عملاء وتمكّنك من التفاعل مع codeforge برمجيا.

يمكنك تشغيل `codeforge serve` لبدء خادم مستقل. إذا كانت واجهة codeforge في terminal (TUI)
قيد التشغيل، فسيبدأ `codeforge serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات CodeForge لـ [IDE](/docs/ide).
...
يعرض خادم codeforge واجهات API التالية.
