...
أنشئ الملف `.codeforge/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات CodeForge أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ CodeForge:
...
- على مستوى النظام: `~/.config/codeforge/commands/`
- لكل مشروع: `.codeforge/commands/`
...
يتضمن codeforge عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
