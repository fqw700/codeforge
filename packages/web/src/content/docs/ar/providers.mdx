---
title: المزوّدون
description: استخدام أي موفّر LLM في CodeForge.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم CodeForge ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات CodeForge.
...
في `~/.local/share/codeforge/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات CodeForge.
...
CodeForge Zen هي قائمة نماذج يوفّرها فريق CodeForge وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع CodeForge. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع CodeForge Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر codeforge، ثم انتقل إلى [codeforge.ai/auth](https://YOUR_DOMAIN.ai/auth).
...
يعمل مثل أي مزوّد آخر في CodeForge واستخدامه اختياري بالكامل.

---

## CodeForge Go

CodeForge Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق CodeForge، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع CodeForge.

1. شغّل الأمر `/connect` في TUI، واختر `CodeForge Go`، ثم انتقل إلى [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen).
...
لاستخدام Amazon Bedrock مع CodeForge:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل codeforge:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `codeforge.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل CodeForge غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل codeforge بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات codeforge.
...
تخزين الرمز في مساحة تخزين مصادقة codeforge.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم CodeForge نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر CodeForge
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`codeforge.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [codeforge-gitlab-auth](https://www.npmjs.com/package/codeforge-gitlab-auth).

##### التهيئة

خصّص عبر `codeforge.json`:
...
لاستخدام اشتراك GitHub Copilot مع codeforge:
...
لاستخدام Google Vertex AI مع CodeForge:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر codeforge، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [codeforge-helicone-session](https://github.com/H2Shami/codeforge-helicone-session) لتسجيل كل محادثة CodeForge تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة CodeForge مدرجة كجلسة مستقلة.
...
يمكنك ضبط codeforge لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط codeforge لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط codeforge لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع CodeForge. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/codeforge) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع CodeForge:
...
3. انسخ مفتاح API لاستخدامه في CodeForge.
...
6. **مهم**: قبل استخدام نماذج السحابة في CodeForge، يجب سحب معلومات النموذج محليا:
...
CodeForge Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق CodeForge. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>CodeForge Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **CodeForge Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ CodeForge.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات codeforge.
...
5. يمكنك أيضا تخصيصها عبر إعدادات codeforge. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات codeforge. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع codeforge. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `codeforge.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ CodeForge بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `codeforge auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات codeforge وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات codeforge.
