---
title: CLI
description: أوامر وخيارات CLI في CodeForge.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ CodeForge افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
codeforge
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع CodeForge برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ CodeForge.
...
يتضمن CLI الخاص بـ CodeForge أيضا الأوامر التالية.

---

### agent

إدارة وكلاء CodeForge.
...
إرفاق terminal بخادم الواجهة الخلفية لـ CodeForge قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
codeforge attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ CodeForge تعمل عن بعد. على سبيل المثال:
...
يعتمد CodeForge على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `codeforge auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/codeforge/auth.json`.

```bash
codeforge auth login
```

عند تشغيل CodeForge يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في CodeForge.
...
تشغيل codeforge في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `codeforge serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| `--attach`   |         | الإرفاق بخادم codeforge قيد التشغيل (مثل http://localhost:4096)    |
...
بدء خادم CodeForge بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
codeforge serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف codeforge دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `codeforge`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات CodeForge لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة CodeForge.

```bash
codeforge import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة CodeForge.
...
بدء خادم CodeForge بدون واجهة مع واجهة ويب.

```bash
codeforge web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى CodeForge عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `codeforge`).
...
إلغاء تثبيت CodeForge وإزالة جميع الملفات ذات الصلة.
...
تحديث codeforge إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر codeforge الخيارات العامة التالية.
...
يمكن تهيئة CodeForge باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `codeforge`) |
