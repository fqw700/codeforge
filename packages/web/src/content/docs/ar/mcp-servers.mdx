...
يمكنك إضافة أدوات خارجية إلى CodeForge باستخدام _Model Context Protocol_، أو MCP. يدعم CodeForge خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات CodeForge](https://YOUR_DOMAIN.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/codeforge` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى CodeForge تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم CodeForge بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك CodeForge المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `codeforge mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن CodeForge الرموز بشكل آمن في `~/.local/share/codeforge/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل CodeForge إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات CodeForge مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط CodeForge بحساب Sentry الخاص بك.
