...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات codeforge. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في codeforge تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات codeforge.
...
يأتي codeforge مع وضعين مدمجين.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.codeforge/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `codeforge.json` لديك:
...
- عالمي: `~/.config/codeforge/modes/`
- للمشروع: `.codeforge/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم codeforge القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات codeforge العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.codeforge/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/codeforge/modes/` لأوضاع عالمية:
