---
title: CLI
description: CodeForge CLI のオプションとコマンド。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodeForge CLI は、引数なしで実行すると、デフォルトで [TUI](/docs/tui) を開始します。

```bash
codeforge
```

ただし、このページに記載されているようにコマンドも受け入れます。これにより、CodeForge をプログラム的に操作できるようになります。
...
CodeForge ターミナルユーザーインターフェイスを開始します。
...
CodeForge CLI には次のコマンドもあります。

---

### agent

CodeForge のエージェントを管理します。
...
`serve` または `web` コマンドを使用して起動された、すでに実行中の CodeForge バックエンドサーバーにターミナルを接続します。

```bash
codeforge attach [url]
```

これにより、リモート CodeForge バックエンドで TUI を使用できるようになります。例えば：
...
CodeForge は [Models.dev](https://models.dev) のプロバイダーリストを利用しているため、`codeforge auth login` を使用して、使用したいプロバイダーの API キーを構成できます。これは `~/.local/share/codeforge/auth.json` に保存されます。

```bash
codeforge auth login
```

CodeForge が起動すると、認証情報ファイルからプロバイダーがロードされます。また、環境またはプロジェクト内の `.env` ファイルで定義されたキーがあるかどうかも確認します。
...
`--refresh` フラグを使用して、キャッシュされたモデルリストを更新します。これは、新しいモデルがプロバイダーに追加され、それを CodeForge で確認したい場合に便利です。
...
プロンプトを直接渡して、非対話モードで codeforge を実行します。
...
実行中の `codeforge serve` インスタンスにアタッチして、実行ごとの MCP サーバーのコールドブート時間を回避することもできます。
...
| `--attach`   |          | 実行中の codeforge サーバー (http://localhost:4096 など) に接続します。                    |
...
API アクセスのためにヘッドレス CodeForge サーバーを起動します。完全な HTTP インターフェイスについては、[サーバーのドキュメント](/docs/server) を確認してください。

```bash
codeforge serve
```

これにより、TUI インターフェイスを使用せずに codeforge 機能への API アクセスを提供する HTTP サーバーが起動します。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `codeforge`)。
...
CodeForge セッションのトークンの使用状況とコストの統計を表示します。
...
JSON ファイルまたは CodeForge 共有 URL からセッションデータをインポートします。

```bash
codeforge import <file>
```

ローカルファイルまたは CodeForge 共有 URL からインポートできます。
...
Web インターフェイスを使用してヘッドレス CodeForge サーバーを起動します。

```bash
codeforge web
```

これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて CodeForge にアクセスします。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `codeforge`)。
...
CodeForge をアンインストールし、関連ファイルをすべて削除します。
...
codeforge を最新バージョンまたは特定のバージョンに更新します。
...
codeforge CLI は次のグローバルフラグを受け取ります。
...
| `OPENCODE_SERVER_USERNAME`            | 文字列   | 基本認証ユーザー名 (デフォルト `codeforge`) をオーバーライドします。 |
