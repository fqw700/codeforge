...
エージェントスキルにより、CodeForge はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
CodeForge は次の場所を検索します。

- プロジェクト設定: `.codeforge/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/codeforge/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、CodeForge は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.codeforge/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/codeforge/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.codeforge/skills/git-release/SKILL.md` を作成します。
...
CodeForge では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`codeforge.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
...
**組み込みエージェントの場合** (`codeforge.json` 内):
