---
title: TUI
description: Using the CodeForge terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodeForge provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running CodeForge starts the TUI for the current directory.
...
When using the CodeForge TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to CodeForge. Allows you to select from available providers and add their API keys.
...
Exit CodeForge. _Aliases_: `/quit`, `/q`
...
This is separate from `codeforge.json`, which configures server/runtime behavior.
