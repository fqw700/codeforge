...
As habilidades do agente permitem que o codeforge descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O codeforge pesquisa nesses locais:

- Configuração do projeto: `.codeforge/skills/<name>/SKILL.md`
- Configuração global: `~/.config/codeforge/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o codeforge sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.codeforge/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/codeforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.codeforge/skills/git-release/SKILL.md` assim:
...
O codeforge lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `codeforge.json`:
...
**Para agentes embutidos** (em `codeforge.json`):
