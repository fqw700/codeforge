---
title: Plugins
description: Escreva seus próprios plugins para estender o codeforge.
---

Plugins permitem que você estenda o codeforge conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do codeforge.
...
- `.codeforge/plugins/` - Plugins em nível de projeto
- `~/.config/codeforge/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/codeforge/node_modules/`.
...
1. Configuração global (`~/.config/codeforge/codeforge.json`)
2. Configuração do projeto (`codeforge.json`)
3. Diretório de plugins global (`~/.config/codeforge/plugins/`)
4. Diretório de plugins do projeto (`.codeforge/plugins/`)
...
O codeforge executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do codeforge para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o codeforge.
...
:::note
Se você estiver usando o aplicativo desktop codeforge, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o codeforge de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao codeforge:
...
O helper `tool` cria uma ferramenta personalizada que o codeforge pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o codeforge junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://YOUR_DOMAIN.ai/docs/sdk) para detalhes.
