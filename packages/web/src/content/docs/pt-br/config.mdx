---
title: Configuração
description: Usando a configuração JSON do codeforge.
---

Você pode configurar o codeforge usando um arquivo de configuração JSON.

---

## Formato

O codeforge suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/codeforge`) - padrões organizacionais
2. **Configuração global** (`~/.config/codeforge/codeforge.json`) - preferências do usuário
3. **Configuração personalizada** (`OPENCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`codeforge.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.codeforge`** - agentes, comandos, plugins
...
:::note
Os diretórios `.codeforge` e `~/.config/codeforge` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/codeforge`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do codeforge em `~/.config/codeforge/codeforge.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/codeforge/tui.json`.
...
Adicione `codeforge.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o codeforge é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OPENCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.codeforge`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.codeforge`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`codeforge.ai/config.json`**](https://YOUR_DOMAIN.ai/config.json).

A configuração do TUI usa [**`codeforge.ai/tui.json`**](https://YOUR_DOMAIN.ai/tui.json).
...
Chaves legadas `theme`, `keybinds` e `tui` em `codeforge.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `codeforge serve` e `codeforge web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor codeforge.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `codeforge.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do codeforge através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o codeforge tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/codeforge/agents/` ou `.codeforge/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o codeforge recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`codeforge run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/codeforge/commands/` ou `.codeforge/commands/`. [Saiba mais aqui](/docs/commands).
...
O codeforge fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o codeforge **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o codeforge com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.codeforge/plugins/` ou `~/.config/codeforge/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o codeforge para usar apenas provedores específicos, em vez de desabilitá-los um a um.
