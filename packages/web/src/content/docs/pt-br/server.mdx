---
title: Servidor
description: Interaja com o servidor codeforge via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `codeforge serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente codeforge pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `codeforge.local` |
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `codeforge`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `codeforge serve` quanto ao `codeforge web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password codeforge serve
```
...
Quando você executa `codeforge`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor codeforge para interagir com o codeforge programaticamente.
:::

Essa arquitetura permite que o codeforge suporte múltiplos clientes e permite que você interaja com o codeforge programaticamente.

Você pode executar `codeforge serve` para iniciar um servidor autônomo. Se você tiver o TUI do codeforge em execução, `codeforge serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do codeforge [IDE](/docs/ide).
...
O servidor codeforge expõe as seguintes APIs.
