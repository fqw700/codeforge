...
Com o codeforge, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o codeforge usa nosso próprio tema `codeforge`.
...
O codeforge vem com vários temas embutidos.
...
- Querem que o codeforge corresponda à aparência do seu terminal
...
O codeforge suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/codeforge/themes/*.json` ou `$XDG_CONFIG_HOME/codeforge/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.codeforge/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.codeforge/themes/*.json`
