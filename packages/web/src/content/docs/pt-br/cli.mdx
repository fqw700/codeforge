---
title: CLI
description: Opções e comandos da CLI do codeforge.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do codeforge, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
codeforge
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o codeforge programaticamente.
...
Inicie a interface de usuário do terminal do codeforge.
...
A CLI do codeforge também possui os seguintes comandos.

---

### agent

Gerencie agentes para o codeforge.
...
Anexe um terminal a um servidor backend do codeforge já em execução, iniciado via comandos `serve` ou `web`.

```bash
codeforge attach [url]
```

Isso permite usar o TUI com um backend codeforge remoto. Por exemplo:
...
O codeforge é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `codeforge auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/codeforge/auth.json`.

```bash
codeforge auth login
```

Quando o codeforge é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no codeforge.
...
Execute o codeforge em modo não interativo passando um prompt diretamente.
...
```bash "codeforge run"
codeforge run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `codeforge serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
codeforge run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| `--attach`   |       | Anexe a um servidor codeforge em execução (por exemplo, http://localhost:4096) |
...
Inicie um servidor codeforge sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
codeforge serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do codeforge sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `codeforge`).
...
Liste todas as sessões do codeforge.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do codeforge.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do codeforge.

```bash
codeforge import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do codeforge.
...
Inicie um servidor codeforge sem cabeça com uma interface web.

```bash
codeforge web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o codeforge através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `codeforge`).
...
Desinstale o codeforge e remova todos os arquivos relacionados.
...
Atualiza o codeforge para a versão mais recente ou uma versão específica.
...
A CLI do codeforge aceita as seguintes flags globais.
...
O codeforge pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `codeforge`) |
