...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do codeforge. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no codeforge permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do codeforge.
...
O codeforge vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.codeforge/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `codeforge.json`:
...
- Global: `~/.config/codeforge/modes/`
- Projeto: `.codeforge/modes/`

```markdown title="~/.config/codeforge/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o codeforge usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do codeforge quanto para a configuração específica do projeto.
...
```json title="codeforge.json" {4-14}
{
  "$schema": "https://YOUR_DOMAIN.ai/config.json",
  "mode": {
    "docs": {
...
Crie arquivos de modo em `.codeforge/modes/` para modos específicos do projeto ou `~/.config/codeforge/modes/` para modos globais:

```markdown title=".codeforge/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/codeforge/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
