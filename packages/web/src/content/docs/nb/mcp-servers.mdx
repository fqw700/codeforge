...
Du kan legge til eksterne verktøy i CodeForge ved å bruke _Model Context Protocol_ eller MCP. CodeForge støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [CodeForge Config](https://YOUR_DOMAIN.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="codeforge.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/codeforge`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="codeforge.jsonc" {15}
{
  "$schema": "https://YOUR_DOMAIN.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
CodeForge håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil CodeForge:
...
Hvis serveren krever autentisering, vil CodeForge be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `codeforge mcp auth <server-name>`.
...
```json title="codeforge.json" {7-11}
{
  "$schema": "https://YOUR_DOMAIN.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil CodeForge lagre tokens sikkert i `~/.local/share/codeforge/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i CodeForge, sammen med innebygde verktøy. Så du kan administrere dem gjennom CodeForge-konfigurasjonen som alle andre verktøy.
...
```json title="codeforge.json" {11, 14-18}
{
  "$schema": "https://YOUR_DOMAIN.ai/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble CodeForge til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver CodeForge-samtale oppført som en separat økt.
