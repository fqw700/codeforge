...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i CodeForge-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i CodeForge lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom CodeForge-konfigurasjonen.
...
CodeForge kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.codeforge/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `codeforge.json` konfigurasjonsfil:
...
- Globalt: `~/.config/codeforge/modes/`
- Prosjekt: `.codeforge/modes/`
...
Hvis ingen temperatur er spesifisert, bruker CodeForge modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale CodeForge-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.codeforge/modes/` for prosjektspesifikke moduser eller `~/.config/codeforge/modes/` for globale moduser:
