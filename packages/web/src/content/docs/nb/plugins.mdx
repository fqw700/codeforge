...
description: Skriv dine egne programtillegg for å utvide CodeForge.
---

Plugins lar deg utvide CodeForge ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til CodeForge.
...
- `.codeforge/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/codeforge/node_modules/`.
...
1. Global konfigurasjon (`~/.config/codeforge/codeforge.json`)
2. Prosjektkonfigurasjon (`codeforge.json`)
3. Global plugin-katalog (`~/.config/codeforge/plugins/`)
4. Prosjektpluginkatalog (`.codeforge/plugins/`)
...
CodeForge kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En CodeForge SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide CodeForge.
...
:::note
Hvis du bruker CodeForge-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre CodeForge fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til CodeForge:
...
`tool`-hjelperen lager et tilpasset verktøy som CodeForge kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for CodeForge sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://YOUR_DOMAIN.ai/docs/sdk) for detaljer.
