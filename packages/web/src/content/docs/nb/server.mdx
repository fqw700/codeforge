---
title: Tjener
description: Samhandle med CodeForge-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codeforge serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en CodeForge-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `codeforge.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `codeforge` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `codeforge serve` og `codeforge web`.
...
Når du kjører `codeforge` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk CodeForge-serveren til å samhandle med CodeForge programmatisk.
:::

Denne arkitekturen lar CodeForge støtte flere klienter og lar deg samhandle med CodeForge programmatisk.

Du kan kjøre `codeforge serve` for å starte en frittstående server. Hvis du har
CodeForge TUI kjører, vil `codeforge serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av CodeForge [IDE](/docs/ide) plugins.
...
codeforge-serveren viser følgende APIer.
