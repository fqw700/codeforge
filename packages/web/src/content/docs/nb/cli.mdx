---
title: CLI
description: CodeForge CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodeForge CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
codeforge
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med CodeForge programmatisk.
...
Start CodeForge-terminalbrukergrensesnittet.
...
Administrer agenter for CodeForge.
...
Koble en terminal til en allerede kjørende CodeForge-backend-server startet via `serve` eller `web` kommandoer.

```bash
codeforge attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern CodeForge-backend. For eksempel:
...
CodeForge drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `codeforge auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/codeforge/auth.json`.

```bash
codeforge auth login
```

Når CodeForge starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i CodeForge.
...
Kjør CodeForge i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `codeforge serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| `--attach`   |      | Koble til en kjørende CodeForge-server (f.eks. http://localhost:4096)     |
...
Start en headless CodeForge-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
codeforge serve
```

Dette starter en HTTP-server som gir API tilgang til CodeForge-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `codeforge`).
...
Vis tokenbruk og kostnadsstatistikk for CodeForge-øktene dine.
...
Importer øktdata fra en JSON-fil eller en CodeForge share-URL.
...
Start en headless CodeForge-server med et webgrensesnitt.

```bash
codeforge web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til CodeForge via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `codeforge`).
...
Avinstaller CodeForge og fjern alle relaterte filer.
...
Oppdaterer CodeForge til siste versjon eller en spesifikk versjon.
...
CodeForge CLI bruker følgende globale flagg.
...
CodeForge kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `codeforge`) |
