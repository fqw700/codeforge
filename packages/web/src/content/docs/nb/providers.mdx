---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i CodeForge.
---

import config from "../../../../config.mjs"
export const console = config.console

CodeForge bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i CodeForge-konfigurasjonen.
...
i `~/.local/share/codeforge/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i CodeForge
konfig.
...
CodeForge Zen er en liste over modeller levert av CodeForge-teamet som har vært
testet og verifisert for å fungere godt med CodeForge. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med CodeForge Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `CodeForge Zen` og gå til [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen).
...
Det fungerer som alle andre leverandører i CodeForge og er helt valgfritt å bruke.

---

## CodeForge Go

CodeForge Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av CodeForge-teamet som har vært
testet og verifisert for å fungere godt med CodeForge.

1. Kjør kommandoen `/connect` i TUI, velg `CodeForge Go`, og gå til [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen).
...
Slik bruker du Amazon Bedrock med CodeForge:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører CodeForge:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `codeforge.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i codeforge støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at CodeForge skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom CodeForge-konfigurasjonen.
...
for å lagre token i CodeForge auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
CodeForge bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse CodeForge
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`codeforge.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [codeforge-gitlab-auth](https://www.npmjs.com/package/codeforge-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `codeforge.json`:
...
Slik bruker du GitHub Copilot-abonnementet med CodeForge:
...
Slik bruker du Google Vertex AI med CodeForge:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører CodeForge.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom CodeForge, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [codeforge-helicone-session](https://github.com/H2Shami/codeforge-helicone-session) plugin for automatisk å logge hver CodeForge-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver CodeForge-samtale oppført som en separat økt.
...
Du kan konfigurere CodeForge for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere CodeForge for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere CodeForge for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for CodeForge. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/codeforge) for detaljer.
:::
...
Slik bruker du Ollama Cloud med CodeForge:
...
3. Kopier API-nøkkelen for bruk i CodeForge.
...
6. **Viktig**: Før du bruker skymodeller i CodeForge, må du hente modellinformasjonen lokalt:
...
CodeForge Zen er en liste over testede og verifiserte modeller levert av CodeForge-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>CodeForge Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **CodeForge Zen**.

   ```txt
   /connect
   ```

3. Skriv inn CodeForge API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom CodeForge-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom CodeForge-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CodeForge:
...
Du kan også tilpasse modeller gjennom CodeForge-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med CodeForge. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `codeforge.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar CodeForge forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `codeforge auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk CodeForge-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i CodeForge-konfigurasjonen.
