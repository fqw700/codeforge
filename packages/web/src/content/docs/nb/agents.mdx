...
Det er to typer agenter i CodeForge; primære agenter og underagenter.
...
CodeForge kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
CodeForge kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

CodeForge kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `codeforge.json`:
...
- Globalt: `~/.config/codeforge/agents/`
- Per prosjekt: `.codeforge/agents/`
...
Hvis ingen temperatur er spesifisert, bruker CodeForge modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale CodeForge-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i CodeForge-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [CodeForge Zen](/docs/zen), vil du bruke `codeforge/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `codeforge models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/YOUR_ORG/codeforge).
:::
