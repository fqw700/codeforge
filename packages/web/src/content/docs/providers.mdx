---
title: Providers
description: Using any LLM provider in CodeForge.
---

import config from "../../../config.mjs"
export const console = config.console

CodeForge uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your CodeForge config.
...
You can customize the providers through the `provider` section in your CodeForge
config.
...
CodeForge Zen is a list of models provided by the CodeForge team that have been
tested and verified to work well with CodeForge. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with CodeForge Zen.
:::

1. Run the `/connect` command in the TUI, select `CodeForge Zen`, and head to [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen).
...
It works like any other provider in CodeForge and is completely optional to use.

---

## CodeForge Go

CodeForge Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the CodeForge team that have been
tested and verified to work well with CodeForge.

1. Run the `/connect` command in the TUI, select `CodeForge Go`, and head to [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen).
...
To use Amazon Bedrock with CodeForge:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running codeforge:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `codeforge.json`:
...
CodeForge. Anthropic explicitly prohibits this.

Previous versions of CodeForge came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in CodeForge with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for codeforge to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your codeforge config.
...
GitLab Duo support in CodeForge is experimental. Features, configuration, and
behavior may change in future releases.
:::

CodeForge integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
CodeForge uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock CodeForge
to only use your own GitLab-hosted instance, add the following to your
`codeforge.json` file. It is also recommended to disable session sharing.
...
More documentation on [codeforge-gitlab-auth](https://www.npmjs.com/package/codeforge-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, CodeForge will:
...
4. Route tool execution requests through CodeForge's permission-gated tool system
...
To use your GitHub Copilot subscription with codeforge:
...
To use Google Vertex AI with CodeForge:
...
2. Set the required environment variables:
   ...
   Set them while running codeforge.
...
In the event you see a feature or model from Helicone that isn't configured automatically through codeforge, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [codeforge-helicone-session](https://github.com/H2Shami/codeforge-helicone-session) plugin to automatically log each CodeForge conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each CodeForge conversation listed as a separate session.
...
You can configure codeforge to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure codeforge to use local models through LM Studio.
...
You can configure codeforge to use local models through Ollama.

:::tip
Ollama can automatically configure itself for CodeForge. See the [Ollama integration docs](https://docs.ollama.com/integrations/codeforge) for details.
:::
...
To use Ollama Cloud with CodeForge:
...
3. Copy the API key for use in CodeForge.
...
6. **Important**: Before using cloud models in CodeForge, you must pull the model information locally:
...
CodeForge Zen is a list of tested and verified models provided by the CodeForge team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>CodeForge Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **CodeForge Zen**.

   ```txt
   /connect
   ```

3. Enter your CodeForge API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your codeforge config.
...
5. You can also customize them through your codeforge config. Here's an example of specifying a provider
...
You can also customize models through your codeforge config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with codeforge. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `codeforge.json` file in your project directory:
...
The `limit` fields allow CodeForge to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `codeforge auth list` to see if the credentials
...
2. For custom providers, check the codeforge config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your codeforge config.
