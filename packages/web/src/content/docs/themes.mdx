...
With CodeForge you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, CodeForge uses our own `codeforge` theme.
...
CodeForge comes with several built-in themes.
...
- Want CodeForge to match their terminal's appearance
...
CodeForge supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/codeforge/themes/*.json` or `$XDG_CONFIG_HOME/codeforge/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.codeforge/themes/*.json`
4. **Current working directory** - Defined in `./.codeforge/themes/*.json`
