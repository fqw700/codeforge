...
:::caution
Modes are now configured through the `agent` option in the codeforge config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in codeforge allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the codeforge config.
...
codeforge comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.codeforge/plans/*.md` to detail the plan itself
...
Configure modes in your `codeforge.json` config file:
...
both the global codeforge config and the project specific config.
...
Create mode files in `.codeforge/modes/` for project-specific modes or `~/.config/codeforge/modes/` for global modes:
