---
title: Serwer
description: Interakcja z serwerem codeforge za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `codeforge serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient codeforge.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `codeforge.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `codeforge` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `codeforge serve`, jak i `codeforge web`.
...
Kiedy uruchomisz `codeforge`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera codeforge do programowej interakcji z codeforge.
:::

Ta architektura umożliwia obsługę wielu klientów przez codeforge i programową interakcję z kodem otwartym.

Możesz uruchomić `codeforge serve`, aby uruchomić samodzielny serwer. Jeśli masz
codeforge TUI działa, `codeforge serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki codeforge [IDE](/docs/ide).
...
Serwer codeforge udostępnia następujące interfejsy API.
