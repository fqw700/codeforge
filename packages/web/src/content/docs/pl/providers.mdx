---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w codeforge.
---

import config from "../../../../config.mjs"
export const console = config.console

codeforge używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji codeforge.
...
w `~/.local/share/codeforge/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim codeforge
config.
...
CodeForge Zen to lista modeli dostarczonych przez zespół codeforge, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z codeforge. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od CodeForge Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `CodeForge Zen` i przejdź do [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen).
...
Działa jak każdy inny dostawca w codeforge i jest całkowicie opcjonalny w użyciu.

---

## CodeForge Go

CodeForge Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół codeforge, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z codeforge.

1. Uruchom polecenie `/connect` w TUI, wybierz `CodeForge Go` i przejdź do [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen).
...
Aby używać Amazon Bedrock z codeforge:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania codeforge:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `codeforge.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w codeforge nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod codeforge działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji codeforge.
...
codeforge używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować codeforge
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`codeforge.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [codeforge-gitlab-auth](https://www.npmjs.com/package/codeforge-gitlab-auth).

##### Konfiguracja

Customize through `codeforge.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem codeforge:
...
Aby używać Google Vertex AI z codeforge:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania codeforge.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą codeforge, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [codeforge-helicone-session](https://github.com/H2Shami/codeforge-helicone-session), aby automatycznie rejestrować każdą konwersację codeforge jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja codeforge będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować codeforge tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować codeforge tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować codeforge tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem codeforge. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/codeforge).
:::
...
Aby korzystać z Ollama Cloud z codeforge:
...
3. Skopiuj klucz API do użycia w codeforge.
...
6. **Ważne**: Przed użyciem modeli chmurowych w codeforge musisz pobrać informacje o modelu lokalnie:
...
CodeForge Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół codeforge. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>CodeForge Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **CodeForge Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API codeforge.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji codeforge.
...
5. Możesz je także dostosować za pomocą konfiguracji codeforge. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z codeforge:
...
Możesz także dostosować modele za pomocą konfiguracji codeforge. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z codeforge. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `codeforge.json` w katalogu projektu:
...
Pola `limit` pozwalają codeforge zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `codeforge auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację codeforge i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji codeforge.
