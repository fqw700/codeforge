...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji codeforge.
...
te lub skonfiguruj własne za pomocą konfiguracji codeforge.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.codeforge/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `codeforge.json`:
...
- Globalnie: `~/.config/codeforge/modes/`
...
Jeśli nie ma wpływu na temperaturę, codeforge stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna codeforge, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.codeforge/modes/` dla trybów zapisanych dla projektu lub `~/.config/codeforge/modes/` dla trybów globalnych:
