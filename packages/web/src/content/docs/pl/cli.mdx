---
title: CLI
description: Opcje i polecenia wiersza poleceń CodeForge.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI CodeForge domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
codeforge
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać CodeForge programowo.
...
Uruchamia interfejs terminalowy (TUI) CodeForge.
...
Interfejs CLI CodeForge zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami CodeForge.
...
Dołącz terminal do już działającego serwera backendu CodeForge uruchomionego za pomocą `serve` lub `web`.

```bash
codeforge attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem CodeForge. Na przykład:
...
CodeForge integruje się z wieloma dostawcami. Możesz użyć `codeforge auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/codeforge/auth.json`.

```bash
codeforge auth login
```

Podczas uruchamiania CodeForge ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w CodeForge.
...
Uruchom CodeForge w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `codeforge serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| `--attach`   |       | Dołącz do działającego serwera CodeForge (np. http://localhost:4096)     |
...
Uruchom serwer CodeForge (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
codeforge serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API CodeForge bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `codeforge`).
...
Lista wszystkich sesji CodeForge.
...
Pokaż statystyki wykorzystania tokenów i sesji CodeForge.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania CodeForge.
...
Uruchom serwer CodeForge z interfejsem internetowym.

```bash
codeforge web
```

Uruchamia to serwer HTTP i udostępnia CodeForge przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `codeforge`).
...
Odinstaluj CodeForge i usuń wszystkie powiązane pliki.
...
Aktualizuj CodeForge do najnowszej lub określonej wersji.
...
Interfejs CLI CodeForge przyjmuje następujące flagi globalne dla każdego polecenia.
...
CodeForge można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `codeforge`)     |
