...
CodeForge Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

:::note
CodeForge Go jest obecnie w fazie beta.
:::

Go działa jak każdy inny dostawca w CodeForge. Subskrybujesz CodeForge Go i
...
CodeForge Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

CodeForge Go działa jak każdy inny dostawca w CodeForge.

1. Logujesz się do **<a href={console}>CodeForge Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `CodeForge Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować CodeForge Go.
:::
...
CodeForge Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji CodeForge
używa formatu `codeforge-go/<model-id>`. Na przykład dla Kimi K2.5 należy użyć
`codeforge-go/kimi-k2.5` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z CodeForge.
