---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć codeforge.
---

Wtyczki umożliwiające wyłączenie codeforge poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania codeforge.
...
- `.codeforge/plugins/` - Wtyczki na poziomie projektu
- `~/.config/codeforge/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/codeforge/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/codeforge/codeforge.json`)
2. Konfiguracja projektu (`codeforge.json`)
3. Globalny katalog wtyczek (`~/.config/codeforge/plugins/`)
4. Katalog wtyczek projektu (`.codeforge/plugins/`)
...
codeforge uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z codeforge do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do codeforge.
...
:::note
Jeśli korzystasz z aplikacji komputerowej codeforge, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj codeforge czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do codeforge:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod codeforge. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://YOUR_DOMAIN.ai/docs/sdk).
