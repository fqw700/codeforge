...
W CodeForge obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
CodeForge zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
CodeForge ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

CodeForge ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `codeforge.json`:
...
- Globalnie: `~/.config/codeforge/agents/`
- W projekcie: `.codeforge/agents/`
...
Jeśli nie zostanie skonfigurowana, CodeForge używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji CodeForge, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji CodeForge w `provider/model-id`. Na przykład, [CodeForge Zen](/docs/zen), użyj `codeforge/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `codeforge models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/YOUR_ORG/codeforge).
:::
