...
Dzięki codeforge możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie codeforge używa naszego własnego motywu `codeforge`.
...
codeforge ma kilka wbudowanych motywów.
...
- Chcą, aby CodeForge pasował do wyglądu ich terminala
...
codeforge obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/codeforge/themes/*.json` lub `$XDG_CONFIG_HOME/codeforge/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.codeforge/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.codeforge/themes/*.json`
