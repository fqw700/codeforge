...
Możesz dodać dodatkowe informacje do codeforge za pomocą _Model Context Protocol_, w skrócie MCP. codeforge obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [codeforge Config](https://YOUR_DOMAIN.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/codeforge`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
codeforge automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, codeforge:
...
Jeśli serwer wymaga uwierzytelnienia, codeforge poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `codeforge mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji codeforge bezpieczne przechowa tokeny w `~/.local/share/codeforge/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w codeforge, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie codeforge, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć codeforge z kontem Sentry.
