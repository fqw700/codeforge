...
Umiejętności agenta pozwalają codeforge odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
codeforge przeszukuje te lokalizacje:

- Project config: `.codeforge/skills/<name>/SKILL.md`
- Global config: `~/.config/codeforge/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu codeforge przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.codeforge/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/codeforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.codeforge/skills/git-release/SKILL.md` w ten sposób:
...
codeforge wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `codeforge.json`:
...
**Dla agentów wbudowanych** (w `codeforge.json`):
