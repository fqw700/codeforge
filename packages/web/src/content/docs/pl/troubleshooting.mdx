...
Aby debugować problemy z codeforge, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\codeforge\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `codeforge --log-level DEBUG`.

---

## Przechowywanie danych

codeforge przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/codeforge/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\codeforge`
...
codeforge Desktop uruchamia w tle lokalny serwer codeforge (sidecar `codeforge-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `CodeForge` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/codeforge/codeforge.jsonc` (lub `~/.config/codeforge/codeforge.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/codeforge/codeforge.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\codeforge\codeforge.jsonc`
...
codeforge może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/codeforge/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\codeforge\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby codeforge mógł ją odbudować.

1. Całkowicie zamknij codeforge Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/codeforge`
- **Linux**: usuń `~/.cache/codeforge` (lub uruchom `rm -rf ~/.cache/codeforge`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\codeforge`

3. Uruchom ponownie codeforge Desktop.

---

### Napraw problemy z połączeniem z serwerem

codeforge Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `codeforge.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows codeforge Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami codeforge.

---

### Brak powiadomień

codeforge Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla codeforge w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie codeforge Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji codeforge Desktop):

- `codeforge.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `codeforge.global.dat` i `codeforge.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z codeforge:
...
3. Upewnij się, że masz najnowszą wersję z `codeforge upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `codeforge models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\codeforge`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. codeforge dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\codeforge`

2. Uruchom ponownie kod codeforge, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to codeforge do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
codeforge wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
