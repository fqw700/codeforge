...
Create `.codeforge/commands/test.md`:
...
You can add custom commands through the CodeForge config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your CodeForge [config](/docs/config):
...
- Per-project: `.codeforge/commands/`
...
codeforge includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
