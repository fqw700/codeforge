...
Puoi aggiungere strumenti esterni a CodeForge usando il _Model Context Protocol_, o MCP. CodeForge supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [CodeForge Config](https://YOUR_DOMAIN.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/codeforge`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
CodeForge gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, CodeForge:
...
Se il server richiede autenticazione, CodeForge ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `codeforge mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, CodeForge memorizzera' i token in modo sicuro in `~/.local/share/codeforge/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in CodeForge insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione CodeForge come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare CodeForge al tuo account Sentry.
