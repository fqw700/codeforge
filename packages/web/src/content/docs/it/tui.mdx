---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di CodeForge.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodeForge fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire CodeForge avvia la TUI per la directory corrente.
...
Quando usi la TUI di CodeForge, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a CodeForge. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da CodeForge. _Alias_: `/quit`, `/q`
...
Questo è separato da `codeforge.json`, che configura il comportamento di server/runtime.
