...
Le skill degli agenti permettono a CodeForge di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
CodeForge cerca in queste posizioni:

- Config di progetto: `.codeforge/skills/<name>/SKILL.md`
- Config globale: `~/.config/codeforge/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, CodeForge risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.codeforge/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/codeforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.codeforge/skills/git-release/SKILL.md` cosi':
...
CodeForge elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `codeforge.json`:
...
**Per agenti integrati** (in `codeforge.json`):
