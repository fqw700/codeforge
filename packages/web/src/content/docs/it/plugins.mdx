---
title: Plugin
description: Scrivi plugin per estendere CodeForge.
---

I plugin ti permettono di estendere CodeForge agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di CodeForge.
...
- `.codeforge/plugins/` - plugin a livello progetto
- `~/.config/codeforge/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/codeforge/node_modules/`.
...
1. Config globale (`~/.config/codeforge/codeforge.json`)
2. Config di progetto (`codeforge.json`)
3. Directory plugin globale (`~/.config/codeforge/plugins/`)
4. Directory plugin di progetto (`.codeforge/plugins/`)
...
CodeForge esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di codeforge per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere codeforge.
...
:::note
Se usi l'app desktop di CodeForge, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a codeforge di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a codeforge:
...
L'helper `tool` crea uno strumento personalizzato che codeforge può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in codeforge insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://YOUR_DOMAIN.ai/docs/sdk) per i dettagli.
