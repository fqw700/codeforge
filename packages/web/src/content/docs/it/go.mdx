...
CodeForge Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

:::note
CodeForge Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in CodeForge. Ti abboni a CodeForge Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare CodeForge.
...
CodeForge Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

CodeForge Go funziona come qualsiasi altro provider in CodeForge.

1. Accedi a **<a href={console}>CodeForge Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `CodeForge Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a CodeForge Go.
:::
...
CodeForge Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua CodeForge config
utilizza il formato `codeforge-go/<model-id>`. Ad esempio, per Kimi K2.5, useresti
`codeforge-go/kimi-k2.5` nella tua configurazione.
...
Abbiamo creato CodeForge Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con CodeForge.
