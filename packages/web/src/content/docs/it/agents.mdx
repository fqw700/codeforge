...
In CodeForge esistono due tipi di agenti: agenti primari e subagenti.
...
CodeForge include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
CodeForge include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

CodeForge include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `codeforge.json`:
...
- Globale: `~/.config/codeforge/agents/`
- Per progetto: `.codeforge/agents/`
...
Se non specifichi una temperature, CodeForge usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di CodeForge sia per la config specifica del progetto.
...
L'ID modello nella configurazione CodeForge usa il formato `provider/model-id`. Per esempio, se stai usando [CodeForge Zen](/docs/zen), useresti `codeforge/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `codeforge models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/YOUR_ORG/codeforge).
:::
