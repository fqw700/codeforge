...
Crea `.codeforge/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di CodeForge o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di CodeForge:
...
- Globale: `~/.config/codeforge/commands/`
- Per progetto: `.codeforge/commands/`
...
codeforge include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
