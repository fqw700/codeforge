...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di codeforge. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in codeforge ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di codeforge.
...
codeforge include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.codeforge/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `codeforge.json`:
...
- Globale: `~/.config/codeforge/modes/`
- Progetto: `.codeforge/modes/`
...
Se non viene specificata alcuna temperatura, codeforge usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di codeforge sia per quella specifica del progetto.
...
Crea file di modalita in `.codeforge/modes/` per modalita specifiche del progetto oppure in `~/.config/codeforge/modes/` per modalita globali:
