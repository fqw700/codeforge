...
ove ili konfigurirajte svoje putem codeforge config.
...
codeforge dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.codeforge/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `codeforge.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/codeforge/modes/`
- Projekat: `.codeforge/modes/`
...
Ako temperatura nije navedena, codeforge koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.codeforge/modes/` za specifične načine rada ili `~/.config/codeforge/modes/` za globalne načine:
