---
title: Dodaci
description: Napišite vlastite dodatke za proširenje CodeForge.
---

Dodaci vam omogućavaju da proširite CodeForge spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja CodeForge.
...
- `.codeforge/plugins/` - Dodaci na nivou projekta
- `~/.config/codeforge/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/codeforge/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/codeforge/codeforge.json`)
2. Konfiguracija projekta (`codeforge.json`)
3. Globalni direktorij dodataka (`~/.config/codeforge/plugins/`)
4. Direktorij dodataka projekta (`.codeforge/plugins/`)
...
CodeForge pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje CodeForge.
...
Spriječite codeforge da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u codeforge:
...
Pomoćnik `tool` kreira prilagođeni alat koji codeforge može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za codeforge zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://YOUR_DOMAIN.ai/docs/sdk) za detalje.
