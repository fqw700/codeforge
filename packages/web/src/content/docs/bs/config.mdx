---
title: Konfiguracija
description: Korištenje CodeForge JSON konfiguracije.
---

Možete konfigurirati CodeForge koristeći JSON konfiguracijski fajl.

---

## Format

CodeForge podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/codeforge`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/codeforge/codeforge.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`codeforge.json` u projektu) - postavke specifične za projekat
5. **`.codeforge` direktoriji** - agenti, komande, dodaci
...
:::note
Direktoriji `.codeforge` i `~/.config/codeforge` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/codeforge` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu CodeForge konfiguraciju u `~/.config/codeforge/codeforge.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/codeforge/tui.json`.
...
Dodajte `codeforge.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se CodeForge pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.codeforge` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.codeforge`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`codeforge.ai/config.json`**](https://YOUR_DOMAIN.ai/config.json).

TUI konfiguracija koristi [**`codeforge.ai/tui.json`**](https://YOUR_DOMAIN.ai/tui.json).
...
Stari `theme`, `keybinds`, i `tui` ključevi u `codeforge.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `codeforge serve` i `codeforge web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš CodeForge server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `codeforge.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj CodeForge konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, CodeForge pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/codeforge/agents/` ili `.codeforge/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, CodeForge će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`codeforge run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/codeforge/commands/` ili `.codeforge/commands/`. [Saznajte više ovdje](/docs/commands).
...
CodeForge će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, CodeForge **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju CodeForge sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.codeforge/plugins/` ili `~/.config/codeforge/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite CodeForge da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
