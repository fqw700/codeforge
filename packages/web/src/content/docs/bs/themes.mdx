...
U CodeForge mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, CodeForge koristi nasu `codeforge` temu.
...
CodeForge dolazi sa vise ugradenih tema.
...
- Zele da CodeForge odgovara izgledu njihovog terminala
...
CodeForge podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/codeforge/themes/*.json` ili `$XDG_CONFIG_HOME/codeforge/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.codeforge/themes/*.json`
4. **Trenutni radni direktorij** - `./.codeforge/themes/*.json`
