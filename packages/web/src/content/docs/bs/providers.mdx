---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u CodeForge.
---

import config from "../../../../config.mjs"
export const console = config.console

CodeForge koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj CodeForge konfiguraciji.
...
u `~/.local/share/codeforge/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem CodeForge
config.
...
CodeForge Zen je lista modela koje je obezbedio CodeForge tim koji su bili
testirano i potvrđeno da dobro radi sa CodeForge. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa CodeForge Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite codeforge i idite na [codeforge.ai/auth](https://YOUR_DOMAIN.ai/auth).
...
Radi kao i svaki drugi provajder u CodeForge i potpuno je opcionalan za korištenje.

---

## CodeForge Go

CodeForge Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža CodeForge tim i koji su testirani i verificirani da dobro rade s CodeForge-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `CodeForge Go` i idite na [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen).
...
Da biste koristili Amazon Bedrock s CodeForge:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete codeforge:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `codeforge.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u CodeForge.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi codeforge ispravno radio.
   :::
...
Također možete dodati modele kroz svoju codeforge konfiguraciju.
...
da pohrani token u codeforge auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
CodeForge koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate CodeForge
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`codeforge.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [codeforge-gitlab-auth](https://www.npmjs.com/package/codeforge-gitlab-auth).

##### Konfiguracija

Prilagodite putem `codeforge.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s codeforge:
...
Za korištenje Google Vertex AI s CodeForge:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće codeforge.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem codeforgea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [codeforge-helicone-session](https://github.com/H2Shami/codeforge-helicone-session) da automatski prijavite svaki CodeForge razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki CodeForge razgovor naveden kao zasebna sesija.
...
Možete konfigurirati codeforge za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati codeforge za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati codeforge za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za CodeForge. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/codeforge) za detalje.
:::
...
Da biste koristili Ollama Cloud s CodeForge:
...
3. Kopirajte API ključ za korištenje u CodeForge.
...
6. **Važno**: Prije upotrebe modela oblaka u CodeForge, morate lokalno povući informacije o modelu:
...
CodeForge Zen je lista testiranih i verifikovanih modela koju je obezbedio CodeForge tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>CodeForge Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **CodeForge Zen**.

```txt
   /connect
```

3. Unesite svoj CodeForge API ključ.
...
Također možete dodati dodatne modele putem vaše codeforge konfiguracije.
...
5. Također ih možete prilagoditi putem vaše codeforge konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju codeforge konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s codeforge-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `codeforge.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju CodeForge da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `codeforge auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite CodeForge konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj codeforge konfiguraciji.
