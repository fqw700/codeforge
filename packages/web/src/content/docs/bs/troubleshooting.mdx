...
Da biste otklonili probleme s CodeForge, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codeforge\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `codeforge --log-level DEBUG`.

---

## Pohrana

CodeForge pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/codeforge/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codeforge`
...
CodeForge Desktop pokreće lokalni CodeForge server (`codeforge-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `CodeForge` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/codeforge/codeforge.jsonc` (ili `~/.config/codeforge/codeforge.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/codeforge/codeforge.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codeforge\codeforge.jsonc`
...
CodeForge također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/codeforge/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codeforge\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga CodeForge mogao ponovo izgraditi.

1. Potpuno zatvorite CodeForge Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/codeforge`
- **Linux**: obrišite `~/.cache/codeforge` (ili pokrenite `rm -rf ~/.cache/codeforge`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\codeforge`

3. Ponovo pokrenite CodeForge Desktop.

---

### Rješavanje problema sa vezom na serveru

CodeForge Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `codeforge.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, CodeForge Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa CodeForge karakteristikama.

---

### Obavještenja se ne prikazuju

CodeForge Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za CodeForge u postavkama vašeg OS-a, i
...
1. Zatvorite CodeForge Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka CodeForge Desktop aplikacije):

- `codeforge.settings.dat` (URL zadanog servera za desktop)
- `codeforge.global.dat` i `codeforge.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s CodeForge:
...
Da saznate kojim modelima imate pristup, pokrenite `codeforge models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\codeforge`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. CodeForge dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\codeforge`

2. Ponovo pokrenite CodeForge da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti CodeForge da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
CodeForge će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
