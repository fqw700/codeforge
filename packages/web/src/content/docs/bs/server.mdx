---
title: Server
description: Komunicirajte s codeforge serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `codeforge serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji codeforge klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `codeforge.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `codeforge`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `codeforge serve` i za `codeforge web`.
...
Kada pokrenete `codeforge`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite codeforge server za programsku interakciju sa codeforge-om.
:::

Ova arhitektura omogucava codeforge podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `codeforge serve` da startate standalone server. Ako je codeforge TUI vec pokrenut, `codeforge serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste CodeForge [IDE](/docs/ide) pluginovi.
...
codeforge server izlaže sljedece API-je.
