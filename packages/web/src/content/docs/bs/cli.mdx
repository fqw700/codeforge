---
title: CLI
description: CodeForge CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodeForge CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
codeforge
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa CodeForge.
...
Pokrenite CodeForge terminalski korisnički interfejs.
...
CodeForge CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte CodeForge agentima.
...
Priključite terminal na već pokrenut CodeForge backend server pokrenut putem `serve` ili `web` naredbi.

```bash
codeforge attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim CodeForge backend-om. Na primjer:
...
CodeForge pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `codeforge auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/codeforge/auth.json`.

```bash
codeforge auth login
```

Kada se CodeForge pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u CodeForge.
...
Pokrenite CodeForge u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `codeforge serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| `--attach`   |        | Priključite na pokrenuti CodeForge server (npr. http://localhost:4096)    |
...
Pokrenite CodeForge headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
codeforge serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti CodeForge-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `codeforge`).
...
Upravljajte CodeForge sesijama.
...
Prikaži statistiku upotrebe tokena i troškova za vaše CodeForge sesije.
...
Uvezite podatke sesije iz JSON datoteke ili CodeForge dijeljenog URL-a.

```bash
codeforge import <file>
```

Možete uvesti iz lokalne datoteke ili CodeForge dijeljenog URL-a.
...
Pokrenite CodeForge headless server sa web interfejsom.

```bash
codeforge web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup CodeForge-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `codeforge`).
...
Deinstalirajte CodeForge i uklonite sve povezane datoteke.
...
Ažurira CodeForge na najnoviju verziju ili određenu verziju.
...
CodeForge CLI prihvata sljedeće globalne zastavice.
...
CodeForge se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `codeforge`) |
