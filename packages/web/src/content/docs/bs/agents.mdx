...
Postoje dvije vrste agenata u CodeForge: primarni agenti i podagenti.
...
CodeForge dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
CodeForge dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

CodeForge dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `codeforge.json`:
...
- Globalno: `~/.config/codeforge/agents/`
- Po projektu: `.codeforge/agents/`
...
Ako temperatura nije navedena, CodeForge koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu CodeForge konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj CodeForge konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [CodeForge Zen](/docs/zen), koristili biste `codeforge/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `codeforge models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/YOUR_ORG/codeforge).
:::
