...
Agent skills omogucavaju CodeForge da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
CodeForge pretrazuje ove lokacije:

- Konfiguracija projekta: `.codeforge/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/codeforge/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, CodeForge ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.codeforge/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/codeforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.codeforge/skills/git-release/SKILL.md` ovako:
...
CodeForge navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `codeforge.json`:
...
**Za ugradene agente** (u `codeforge.json`):
