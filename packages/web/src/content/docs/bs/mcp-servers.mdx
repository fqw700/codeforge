...
Možete dodati vanjske alate u CodeForge koristeći _Model Context Protocol_, ili MCP. CodeForge podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [CodeForge Config](https://YOUR_DOMAIN.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/codeforge`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
CodeForge automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, CodeForge će:
...
Ako server zahtijeva autentifikaciju, CodeForge će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `codeforge mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, CodeForge sigurno čuva tokene u `~/.local/share/codeforge/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u CodeForge, zajedno s ugrađenim alatima. Možete njima upravljati kroz CodeForge konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete CodeForge sa vašim Sentry nalogom.
