...
CodeForge Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
CodeForge Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u CodeForge-u. Pretplatite se na CodeForge Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili CodeForge.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa CodeForge-om.
:::
...
CodeForge Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

CodeForge Go radi kao bilo koji drugi provajder u CodeForge-u.

1. Prijavite se na **<a href={console}>CodeForge Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `CodeForge Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na CodeForge Go.
:::
...
CodeForge Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj CodeForge konfiguraciji
koristi format `codeforge-go/<model-id>`. Na primjer, za Kimi K2.5, koristili biste
`codeforge-go/kimi-k2.5` u svojoj konfiguraciji.
...
Napravili smo CodeForge Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz CodeForge koristite i bilo kojeg drugog provajdera.
