...
Agentferdigheter lar CodeForge oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
CodeForge søker etter disse stedene:

- Prosjektkonfigurasjon: `.codeforge/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/codeforge/skills/<name>/SKILL.md`
...
For prosjektlokale stier går CodeForge opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.codeforge/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/codeforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.codeforge/skills/git-release/SKILL.md` slik:
...
CodeForge viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `codeforge.json`:
...
**For innebygde agenter** (i `codeforge.json`):
