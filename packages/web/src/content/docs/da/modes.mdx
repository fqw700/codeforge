...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i codeforge-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i codeforge giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem codeforge-konfigurationen.
...
codeforge leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.codeforge/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `codeforge.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger codeforge modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale codeforge-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.codeforge/modes/` for projektspecifikke tilstande eller `~/.config/codeforge/modes/` for globale tilstande:
