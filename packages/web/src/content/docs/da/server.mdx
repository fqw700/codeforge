---
title: Server
description: Interager med codeforge-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codeforge serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en codeforge-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `codeforge.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `codeforge`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `codeforge serve` og `codeforge web`.
...
Når du kører `codeforge` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug codeforge-serveren til at interagere med codeforge programmatisk.
:::

Denne arkitektur lader codeforge understøtte flere klienter og lader dig interagere med codeforge programmatisk.

Du kan køre `codeforge serve` for at starte en selvstændig server. Hvis du har
codeforge TUI kørende, vil `codeforge serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af CodeForge [IDE](/docs/ide) plugins.
...
CodeForge-serveren viser følgende API'er.
