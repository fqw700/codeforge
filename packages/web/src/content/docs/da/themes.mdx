...
Med CodeForge kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger CodeForge vårt eget `codeforge`-tema.
...
CodeForge kommer med flere innebygde temaer.
...
- Vil at CodeForge skal matche terminalens utseende
...
CodeForge støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/codeforge/themes/*.json` eller `$XDG_CONFIG_HOME/codeforge/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.codeforge/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.codeforge/themes/*.json`
