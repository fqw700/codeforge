...
For at fejlfinde problemer med CodeForge, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\codeforge\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `codeforge --log-level DEBUG`.

---

## Lagring

codeforge gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/codeforge/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\codeforge`
...
CodeForge Desktop kører en lokal CodeForge-server (`codeforge-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `CodeForge`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux** (ældre installationer): `~/.local/share/codeforge/codeforge.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\codeforge\codeforge.jsonc`
...
CodeForge kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/codeforge/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\codeforge\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så CodeForge kan genopbygge den.

1. Afslut CodeForge Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/codeforge`
- **Linux**: slet `~/.cache/codeforge` (eller kør `rm -rf ~/.cache/codeforge`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\codeforge`

3. Start CodeForge Desktop på ny.

---

### Løs problemer med serverforbindelse

CodeForge Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `codeforge.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver CodeForge Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i CodeForge.

---

### Meddelelser vises ikke

CodeForge Desktop viser kun systemvarsler når:

- varsler er aktiveret for CodeForge i dine OS-indstillinger, og
...
1. Afslut CodeForge Desktop.
2. Find og slet disse filer (de findes i CodeForge Desktop-appens datamappe):

- `codeforge.settings.dat` (skrivebordsstandardserver URL)
- `codeforge.global.dat` og `codeforge.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med CodeForge:
...
For at finde ud af hvilke modeller du har adgang til, kør `codeforge models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\codeforge`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. codeforge installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\codeforge`

2. Start codeforge på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge codeforge til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
codeforge vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
