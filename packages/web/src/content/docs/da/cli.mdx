---
title: CLI
description: CodeForge CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodeForge CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
codeforge
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med CodeForge programmatisk.
...
Start CodeForge terminalbrugergrænsefladen.
...
CodeForge CLI har også følgende kommandoer.

---

### agent

Administrer agenter i CodeForge.
...
Tilslut en terminal til en allerede kørende CodeForge backend-server startet via `serve` eller `web` kommandoer.

```bash
codeforge attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern CodeForge backend. For eksempel:
...
CodeForge drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `codeforge auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/codeforge/auth.json`.

```bash
codeforge auth login
```

Når CodeForge starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i CodeForge.
...
Kør codeforge i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `codeforge serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| `--attach`   |      | Tilslut til en kørende codeforge-server (f.eks. http://localhost:4096)               |
...
Start en hovedløs CodeForge-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
codeforge serve
```

Dette starter en HTTP-server, der giver API-adgang til codeforge-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `codeforge`).
...
Administrator CodeForge sessionsholder.
...
Liste over alle CodeForge sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine CodeForge-sessioner.
...
Importer sessionsdata fra en JSON fil eller CodeForge del URL.

```bash
codeforge import <file>
```

Du kan importere fra en lokal fil eller en CodeForge share URL.
...
Start en hovedløs CodeForge-server med en webgrænseflade.

```bash
codeforge web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til CodeForge via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `codeforge`).
...
Afinstaller CodeForge og fjern alle relaterede filer.
...
Opdaterer CodeForge til den nyeste version eller en specifik version.
...
codeforge CLI tager følgende globale flag.
...
CodeForge kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `codeforge`) |
