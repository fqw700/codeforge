---
title: Plugins
description: Skriv dine egne plugins for at udvide CodeForge.
---

Plugins giver dig mulighed for at udvide CodeForge ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre CodeForges standardadfærd.
...
- `.codeforge/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/codeforge/node_modules/`.
...
1. Global konfiguration (`~/.config/codeforge/codeforge.json`)
2. Projektkonfiguration (`codeforge.json`)
3. Global plugin-mappe (`~/.config/codeforge/plugins/`)
4. Projekt plugin bibliotek (`.codeforge/plugins/`)
...
CodeForge kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En codeforge SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide codeforge.
...
:::note
Hvis du bruger CodeForge desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå codeforge i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til codeforge:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som codeforge kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for codeforge sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://YOUR_DOMAIN.ai/docs/sdk) for detaljer.
