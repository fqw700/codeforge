---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i CodeForge.
---

import config from "../../../../config.mjs"
export const console = config.console

CodeForge bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i CodeForge-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/codeforge/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i CodeForge-konfigurationen.
...
CodeForge Zen er en liste over modeller leveret af CodeForge-teamet, som er blevet testet og verificeret til at fungere godt med CodeForge. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med CodeForge Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg codeforge og gå til [codeforge.ai/auth](https://YOUR_DOMAIN.ai/auth).
...
Det fungerer som alle andre udbydere i CodeForge og er helt valgfrit at bruge.

---

## CodeForge Go

CodeForge Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af CodeForge-teamet, som er testet og verificeret til at fungere godt med CodeForge.

1. Kør kommandoen `/connect` i TUI, vælg `CodeForge Go`, og gå til [codeforge.ai/auth](https://YOUR_DOMAIN.ai/zen).
...
Sådan bruger du Amazon Bedrock med CodeForge:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører codeforge:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `codeforge.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i CodeForge understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at codeforge skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem codeforge-konfigurationen.
...
gemme tokenet i codeforge auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
CodeForge bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse CodeForge
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`codeforge.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [codeforge-gitlab-auth](https://www.npmjs.com/package/codeforge-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `codeforge.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med CodeForge:
...
Sådan bruger du Google Vertex AI med CodeForge:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører codeforge.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem codeforge, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [codeforge-helicone-session](https://github.com/H2Shami/codeforge-helicone-session) plugin for automatisk at logge hver CodeForge-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver CodeForge-samtale opført som en separat session.
...
Du kan konfigurere codeforge til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere codeforge til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere codeforge til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for CodeForge. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/codeforge) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med CodeForge:
...
3. Kopier API-nøglen til brug i CodeForge.
...
6. **Vigtigt**: Før du bruger skymodeller i CodeForge, skal du hente modelinformationen lokalt:
...
CodeForge Zen er en liste over testede og verificerede modeller leveret af CodeForge-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>CodeForge Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **CodeForge Zen**.

   ```txt
   /connect
   ```

3. Indtast CodeForge API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem codeforge-konfigurationen.
...
5. Du kan også tilpasse dem gennem codeforge-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CodeForge:
...
Du kan også tilpasse modeller gennem codeforge-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med codeforge. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `codeforge.json`-filen i projektkataloget:
...
`limit`-felterne lader CodeForge forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `codeforge auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek codeforge-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i codeforge-konfigurationen.
