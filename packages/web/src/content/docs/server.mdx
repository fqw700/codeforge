---
title: Server
description: Interact with codeforge server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `codeforge serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an codeforge client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `codeforge.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `codeforge`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `codeforge serve` and `codeforge web`.
...
When you run `codeforge` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the codeforge server to interact with codeforge programmatically.
:::

This architecture lets codeforge support multiple clients and allows you to interact with codeforge programmatically.

You can run `codeforge serve` to start a standalone server. If you have the
codeforge TUI running, `codeforge serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CodeForge [IDE](/docs/ide) plugins.
...
The codeforge server exposes the following APIs.
