...
Создайте `.codeforge/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию codeforge или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем codeforge [config](/docs/config):
...
- Глобальный: `~/.config/codeforge/commands/`
- Для каждого проекта: `.codeforge/commands/`
...
codeforge включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
