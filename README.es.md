...
npm i -g codeforge-ai@latest        # o bun/pnpm/yarn
scoop install codeforge             # Windows
choco install codeforge             # Windows
brew install anomalyco/tap/codeforge # macOS y Linux (recomendado, siempre al día)
brew install codeforge              # macOS y Linux (fórmula oficial de brew, se actualiza menos)
sudo pacman -S codeforge            # Arch Linux (Stable)
paru -S codeforge-bin               # Arch Linux (Latest from AUR)
mise use -g codeforge               # cualquier sistema
nix run nixpkgs#codeforge           # o github:YOUR_ORG/codeforge para la rama dev más reciente
```
...
CodeForge también está disponible como aplicación de escritorio. Descárgala directamente desde la [página de releases](https://github.com/YOUR_ORG/codeforge/releases) o desde [codeforge.ai/download](https://YOUR_DOMAIN.ai/download).
...
4. `$HOME/.codeforge/bin` - Alternativa por defecto
...
CodeForge incluye dos agents integrados que puedes alternar con la tecla `Tab`.
...
Más información sobre [agents](https://YOUR_DOMAIN.ai/docs/agents).

### Documentación

Para más información sobre cómo configurar CodeForge, [**ve a nuestra documentación**](https://YOUR_DOMAIN.ai/docs).

### Contribuir

Si te interesa contribuir a CodeForge, lee nuestras [docs de contribución](./CONTRIBUTING.md) antes de enviar un pull request.

### Construyendo sobre CodeForge

Si estás trabajando en un proyecto relacionado con CodeForge y usas "codeforge" como parte del nombre; por ejemplo, "codeforge-dashboard" u "codeforge-mobile", agrega una nota en tu README para aclarar que no está construido por el equipo de CodeForge y que no está afiliado con nosotros de ninguna manera.
...
- 100% open source
- No está acoplado a ningún proveedor. Aunque recomendamos los modelos que ofrecemos a través de [CodeForge Zen](https://YOUR_DOMAIN.ai/zen); CodeForge se puede usar con Claude, OpenAI, Google o incluso modelos locales. A medida que evolucionan los modelos, las brechas se cerrarán y los precios bajarán, por lo que ser agnóstico al proveedor es importante.
- Soporte LSP listo para usar
- Un enfoque en la TUI. CodeForge está construido por usuarios de neovim y los creadores de [terminal.shop](https://terminal.shop); vamos a empujar los límites de lo que es posible en la terminal.
- Arquitectura cliente/servidor. Esto, por ejemplo, permite ejecutar CodeForge en tu computadora mientras lo controlas de forma remota desde una app móvil. Esto significa que el frontend TUI es solo uno de los posibles clientes.

---

**Únete a nuestra comunidad** [Discord](https://discord.gg/codeforge) | [X.com](https://x.com/codeforge)
