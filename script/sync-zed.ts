...
const EXTENSION_NAME = "codeforge"
...
async function main() {
  ...
  // Delete any existing branches for codeforge updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating CodeForge extension to v${cleanVersion}"`
